...
@Getter
@Setter
@Table(name = "t_item_group", indexes = {
        @Index(name = "idx_lft", columnList = "ct_lft"),
        @Index(name = "idx_rgt", columnList = "ct_rgt"),
        @Index(name = "idx_modified", columnList = "dtt_modified"),
}, uniqueConstraints = {
        @UniqueConstraint(name = "uk_item_group_name", columnNames = {"tx_item_group_name"})
})
public class ItemGroupDao {
    ...
    @Column(name = "tx_item_group_name", unique = true, length = 140)
    private String itemGroupName;

    @Column(name = "tx_parent_item_group", length = 140)
