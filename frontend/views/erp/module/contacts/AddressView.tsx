...
import AddressDto from 'Frontend/generated/com/itbd/application/db/dto/address/AddressDto';
import AddressDtoModel from 'Frontend/generated/com/itbd/application/db/dto/address/AddressDtoModel';
...
import { AddressDtoCrudService, DocFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function AddressView() {
  ...
  const [user, setUser] = useState<AddressDto>({} as AddressDto);
  const [selectedUserItems, setSelectedUserItems] = useState<AddressDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(AddressDtoModel, {
    onSubmit: async (userE) => {
      await AddressDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: AddressDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: AddressDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          AddressDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Address')).then(
      (result) => {
        setUiField(result);
      }
    );
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={AddressDtoCrudService}
              model={AddressDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'addressType', 'city', 'country', 'creation', 'idx']}
