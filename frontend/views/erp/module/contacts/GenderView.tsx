...
import GenderDto from 'Frontend/generated/com/itbd/application/db/dto/GenderDto';
import GenderDtoModel from 'Frontend/generated/com/itbd/application/db/dto/GenderDtoModel';
...
import { DocFieldDtoCrudService, GenderDtoCrudService } from 'Frontend/generated/endpoints';
...
function GenderView() {
  ...
  const [user, setUser] = useState<GenderDto>({} as GenderDto);
  const [selectedUserItems, setSelectedUserItems] = useState<GenderDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(GenderDtoModel, {
    onSubmit: async (userE) => {
      await GenderDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Gender')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: GenderDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function DeleteRander({ item }: { item: GenderDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          GenderDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={GenderDtoCrudService}
              model={GenderDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'gender', 'idx']}
