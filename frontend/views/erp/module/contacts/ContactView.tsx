...
import ContactDto from 'Frontend/generated/com/itbd/application/db/dto/contacts/ContactDto';
import ContactDtoModel from 'Frontend/generated/com/itbd/application/db/dto/contacts/ContactDtoModel';
...
import { ContactDtoCrudService, DocFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function ContactView() {
  ...
  const [user, setUser] = useState<ContactDto>({} as ContactDto);
  const [selectedUserItems, setSelectedUserItems] = useState<ContactDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ContactDtoModel, {
    onSubmit: async (userE) => {
      await ContactDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: ContactDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: ContactDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          ContactDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Contact')).then(
      (result) => {
        setUiField(result);
      }
    );
