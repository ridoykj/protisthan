...
import SalutationDto from 'Frontend/generated/com/itbd/application/db/dto/SalutationDto';
import SalutationDtoModel from 'Frontend/generated/com/itbd/application/db/dto/SalutationDtoModel';
...
import { DocFieldDtoCrudService, SalutationDtoCrudService } from 'Frontend/generated/endpoints';
...
function SalutationView() {
  ...
  const [user, setUser] = useState<SalutationDto>({} as SalutationDto);
  const [selectedUserItems, setSelectedUserItems] = useState<SalutationDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(SalutationDtoModel, {
    onSubmit: async (userE) => {
      await SalutationDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Salutation')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: SalutationDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function DeleteRander({ item }: { item: SalutationDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          SalutationDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={SalutationDtoCrudService}
              model={SalutationDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'salutation', 'idx']}
