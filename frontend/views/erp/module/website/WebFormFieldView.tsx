...
import WebFormFieldDto from 'Frontend/generated/com/itbd/application/db/dto/website/component/WebFormFieldDto';
import WebFormFieldDtoModel from 'Frontend/generated/com/itbd/application/db/dto/website/component/WebFormFieldDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, WebFormFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function WebFormFieldView() {
  ...
  const [user, setUser] = useState<WebFormFieldDto>({} as WebFormFieldDto);
  const [selectedUserItems, setSelectedUserItems] = useState<WebFormFieldDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(WebFormFieldDtoModel, {
    onSubmit: async (userE) => {
      await WebFormFieldDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: WebFormFieldDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/web-form-field/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: WebFormFieldDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          WebFormFieldDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={WebFormFieldDtoCrudService}
              model={WebFormFieldDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'fieldname', 'fieldtype', 'label', 'reqd', 'options', 'idx']}
...
export default WebFormFieldView;
