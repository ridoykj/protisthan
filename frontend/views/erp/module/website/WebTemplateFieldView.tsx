...
import WebTemplateFieldDto from 'Frontend/generated/com/itbd/application/db/dto/website/component/WebTemplateFieldDto';
import WebTemplateFieldDtoModel from 'Frontend/generated/com/itbd/application/db/dto/website/component/WebTemplateFieldDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, WebTemplateFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function WebTemplateFieldView() {
  ...
  const [user, setUser] = useState<WebTemplateFieldDto>({} as WebTemplateFieldDto);
  const [selectedUserItems, setSelectedUserItems] = useState<WebTemplateFieldDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(WebTemplateFieldDtoModel, {
    onSubmit: async (userE) => {
      await WebTemplateFieldDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as WebTemplateFieldDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: WebTemplateFieldDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/web-template-field/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: WebTemplateFieldDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          WebTemplateFieldDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={WebTemplateFieldDtoCrudService}
              model={WebTemplateFieldDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'label', 'fieldname', 'fieldtype', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/web-template-field/_`);
        }}
      >
...
export default WebTemplateFieldView;
