...
import WebTemplateDto from 'Frontend/generated/com/itbd/application/db/dto/website/component/WebTemplateDto';
import WebTemplateDtoModel from 'Frontend/generated/com/itbd/application/db/dto/website/component/WebTemplateDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, WebTemplateDtoCrudService } from 'Frontend/generated/endpoints';
...
function WebTemplateView() {
  ...
  const [user, setUser] = useState<WebTemplateDto>({} as WebTemplateDto);
  const [selectedUserItems, setSelectedUserItems] = useState<WebTemplateDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(WebTemplateDtoModel, {
    onSubmit: async (userE) => {
      await WebTemplateDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: WebTemplateDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: WebTemplateDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          WebTemplateDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={WebTemplateDtoCrudService}
              model={WebTemplateDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'type', 'standard', 'idx']}
