...
import WebFormDto from 'Frontend/generated/com/itbd/application/db/dto/website/component/WebFormDto';
import WebFormDtoModel from 'Frontend/generated/com/itbd/application/db/dto/website/component/WebFormDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, WebFormDtoCrudService } from 'Frontend/generated/endpoints';
...
function WebFormView() {
  ...
  const [user, setUser] = useState<WebFormDto>({} as WebFormDto);
  const [selectedUserItems, setSelectedUserItems] = useState<WebFormDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(WebFormDtoModel, {
    onSubmit: async (userE) => {
      await WebFormDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: WebFormDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: WebFormDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          WebFormDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={WebFormDtoCrudService}
              model={WebFormDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'route', 'published', 'docType', 'idx']}
