...
import WebsiteThemeDto from 'Frontend/generated/com/itbd/application/db/dto/website/WebsiteThemeDto';
import WebsiteThemeDtoModel from 'Frontend/generated/com/itbd/application/db/dto/website/WebsiteThemeDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, WebsiteThemeDtoCrudService } from 'Frontend/generated/endpoints';
...
function WebsiteThemeView() {
  ...
  const [user, setUser] = useState<WebsiteThemeDto>({} as WebsiteThemeDto);
  const [selectedUserItems, setSelectedUserItems] = useState<WebsiteThemeDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(WebsiteThemeDtoModel, {
    onSubmit: async (userE) => {
      await WebsiteThemeDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: WebsiteThemeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: WebsiteThemeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          WebsiteThemeDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={WebsiteThemeDtoCrudService}
              model={WebsiteThemeDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'theme', 'module', 'themeUrl', 'idx']}
...
export default WebsiteThemeView;
