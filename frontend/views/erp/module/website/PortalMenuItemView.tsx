...
import PortalMenuItemDto from 'Frontend/generated/com/itbd/application/db/dto/PortalMenuItemDto';
import PortalMenuItemDtoModel from 'Frontend/generated/com/itbd/application/db/dto/PortalMenuItemDtoModel';
...
import { DocFieldDtoCrudService, PortalMenuItemDtoCrudService } from 'Frontend/generated/endpoints';
...
function PortalMenuItemView() {
  ...
  const [user, setUser] = useState<PortalMenuItemDto>({} as PortalMenuItemDto);
  const [selectedUserItems, setSelectedUserItems] = useState<PortalMenuItemDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(PortalMenuItemDtoModel, {
    onSubmit: async (userE) => {
      await PortalMenuItemDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: PortalMenuItemDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/portal-menu-item/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: PortalMenuItemDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          PortalMenuItemDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={PortalMenuItemDtoCrudService}
              model={PortalMenuItemDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'title', 'enabled', 'route', 'referenceDoctype', 'role', 'idx']}
...
export default PortalMenuItemView;
