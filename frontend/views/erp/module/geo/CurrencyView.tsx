...
import CurrencyDto from 'Frontend/generated/com/itbd/application/db/dto/currencys/CurrencyDto';
import CurrencyDtoModel from 'Frontend/generated/com/itbd/application/db/dto/currencys/CurrencyDtoModel';
...
import { CurrencyDtoCrudService, DocFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function CurrencyView() {
  ...
  const [user, setUser] = useState<CurrencyDto>({} as CurrencyDto);
  const [selectedUserItems, setSelectedUserItems] = useState<CurrencyDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(CurrencyDtoModel, {
    onSubmit: async (userE) => {
      await CurrencyDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Currency')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: CurrencyDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: CurrencyDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          CurrencyDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={CurrencyDtoCrudService}
              model={CurrencyDtoModel}
              ref={autoGridRef}
