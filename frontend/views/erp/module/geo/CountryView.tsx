...
import CountryDto from 'Frontend/generated/com/itbd/application/db/dto/CountryDto';
import CountryDtoModel from 'Frontend/generated/com/itbd/application/db/dto/CountryDtoModel';
...
import { CountryDtoCrudService, DocFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function CountryView() {
  ...
  const [user, setUser] = useState<CountryDto>({} as CountryDto);
  const [selectedUserItems, setSelectedUserItems] = useState<CountryDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(CountryDtoModel, {
    onSubmit: async (userE) => {
      await CountryDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Country')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: CountryDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: CountryDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          CountryDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={CountryDtoCrudService}
              model={CountryDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'countryName', 'dateFormat', 'timeFormat', 'timeZones', 'code', 'idx']}
