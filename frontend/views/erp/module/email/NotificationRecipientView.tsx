...
import NotificationRecipientDto from 'Frontend/generated/com/itbd/application/db/dto/notifications/NotificationRecipientDto';
import NotificationRecipientDtoModel from 'Frontend/generated/com/itbd/application/db/dto/notifications/NotificationRecipientDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, NotificationRecipientDtoCrudService } from 'Frontend/generated/endpoints';
...
function NotificationRecipientView() {
  ...
  const [user, setUser] = useState<NotificationRecipientDto>({} as NotificationRecipientDto);
  const [selectedUserItems, setSelectedUserItems] = useState<NotificationRecipientDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(NotificationRecipientDtoModel, {
    onSubmit: async (userE) => {
      await NotificationRecipientDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Notification Recipient')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as NotificationRecipientDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: NotificationRecipientDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/notification-recipient/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: NotificationRecipientDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          NotificationRecipientDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={NotificationRecipientDtoCrudService}
              model={NotificationRecipientDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'receiverByDocumentField', 'receiverByRole', 'condition', 'idx']}
              ...
              columnOptions={{
                ...
                receiverByDocumentField: {
                  header: 'Receiver By Document Field',
                  resizable: true,
                },
                receiverByRole: {
                  header: 'Receiver By Role',
                  resizable: true,
                },
                condition: {
                  header: 'Condition',
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/notification-recipient/_`);
        }}
      >
