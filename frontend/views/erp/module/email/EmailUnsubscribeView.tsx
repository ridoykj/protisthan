...
import EmailUnsubscribeDto from 'Frontend/generated/com/itbd/application/db/dto/emails/EmailUnsubscribeDto';
import EmailUnsubscribeDtoModel from 'Frontend/generated/com/itbd/application/db/dto/emails/EmailUnsubscribeDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, EmailUnsubscribeDtoCrudService } from 'Frontend/generated/endpoints';
...
function EmailUnsubscribeView() {
  ...
  const [user, setUser] = useState<EmailUnsubscribeDto>({} as EmailUnsubscribeDto);
  const [selectedUserItems, setSelectedUserItems] = useState<EmailUnsubscribeDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(EmailUnsubscribeDtoModel, {
    onSubmit: async (userE) => {
      await EmailUnsubscribeDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Email Unsubscribe')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as EmailUnsubscribeDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: EmailUnsubscribeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/email-unsubscribe/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: EmailUnsubscribeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          EmailUnsubscribeDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={EmailUnsubscribeDtoCrudService}
              model={EmailUnsubscribeDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'email', 'referenceDoctype', 'referenceName', 'globalUnsubscribe', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/email-unsubscribe/_`);
        }}
      >
