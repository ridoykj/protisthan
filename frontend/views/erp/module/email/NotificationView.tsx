...
import NotificationDto from 'Frontend/generated/com/itbd/application/db/dto/notifications/NotificationDto';
import NotificationDtoModel from 'Frontend/generated/com/itbd/application/db/dto/notifications/NotificationDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, NotificationDtoCrudService } from 'Frontend/generated/endpoints';
...
function NotificationView() {
  ...
  const [user, setUser] = useState<NotificationDto>({} as NotificationDto);
  const [selectedUserItems, setSelectedUserItems] = useState<NotificationDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(NotificationDtoModel, {
    onSubmit: async (userE) => {
      await NotificationDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Notification')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: NotificationDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function DeleteRander({ item }: { item: NotificationDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          NotificationDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={NotificationDtoCrudService}
              model={NotificationDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'subject', 'documentType', 'module', 'event', 'condition', 'idx']}
