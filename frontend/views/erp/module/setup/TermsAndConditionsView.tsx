...
import TermsAndConditionsDto from 'Frontend/generated/com/itbd/application/db/dto/TermsAndConditionsDto';
import TermsAndConditionsDtoModel from 'Frontend/generated/com/itbd/application/db/dto/TermsAndConditionsDtoModel';
...
import {
  ...
  TermsAndConditionsDtoCrudService,
...
function TermsAndConditionsView() {
  ...
  const [user, setUser] = useState<TermsAndConditionsDto>({} as TermsAndConditionsDto);
  const [selectedUserItems, setSelectedUserItems] = useState<TermsAndConditionsDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(TermsAndConditionsDtoModel, {
    onSubmit: async (userE) => {
      await TermsAndConditionsDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: TermsAndConditionsDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: TermsAndConditionsDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          TermsAndConditionsDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Terms and conditions')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as TermsAndConditionsDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={TermsAndConditionsDtoCrudService}
              model={TermsAndConditionsDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'title', 'selling', 'buying', 'terms', 'idx']}
