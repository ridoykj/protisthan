...
import DesignationDto from 'Frontend/generated/com/itbd/application/db/dto/DesignationDto';
import DesignationDtoModel from 'Frontend/generated/com/itbd/application/db/dto/DesignationDtoModel';
...
import { DesignationDtoCrudService, DocFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function DesignationView() {
  ...
  const [user, setUser] = useState<DesignationDto>({} as DesignationDto);
  const [selectedUserItems, setSelectedUserItems] = useState<DesignationDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(DesignationDtoModel, {
    onSubmit: async (userE) => {
      await DesignationDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Designation')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: DesignationDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: DesignationDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          DesignationDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={DesignationDtoCrudService}
              model={DesignationDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'designationName', 'idx']}
