...
import TerritoryDto from 'Frontend/generated/com/itbd/application/db/dto/TerritoryDto';
import TerritoryDtoModel from 'Frontend/generated/com/itbd/application/db/dto/TerritoryDtoModel';
...
function TerritoryView() {
  ...
  const [user, setUser] = useState<TerritoryDto>({} as TerritoryDto);
  const [selectedUserItems, setSelectedUserItems] = useState<TerritoryDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(TerritoryDtoModel, {
    onSubmit: async (userE) => {
      await TerritoryDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'territory')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: TerritoryDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: TerritoryDto }) {
    return (
      <button
        type="button"
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={TerritoryDtoCrudService}
              model={TerritoryDtoModel}
              ref={autoGridRef}
