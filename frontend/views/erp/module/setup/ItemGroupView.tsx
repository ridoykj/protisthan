...
import ItemGroupDto from 'Frontend/generated/com/itbd/application/db/dto/items/ItemGroupDto';

import ItemGroupDtoModel from 'Frontend/generated/com/itbd/application/db/dto/items/ItemGroupDtoModel';
...
function ItemGroupView() {
  ...
  const [user, setUser] = useState<ItemGroupDto>({} as ItemGroupDto);
  const [selectedUserItems, setSelectedUserItems] = useState<ItemGroupDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ItemGroupDtoModel, {
    onSubmit: async (itemE) => {
      await ItemGroupDtoCrudService.save(itemE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Item group')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: ItemGroupDto }) {
    const { name } = item;
  ...
  function deleteRander({ item }: { item: ItemGroupDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          ItemGroupDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
