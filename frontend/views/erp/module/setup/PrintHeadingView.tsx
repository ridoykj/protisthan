...
import PrintHeadingDto from 'Frontend/generated/com/itbd/application/db/dto/print/PrintHeadingDto';
import PrintHeadingDtoModel from 'Frontend/generated/com/itbd/application/db/dto/print/PrintHeadingDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, PrintHeadingDtoCrudService } from 'Frontend/generated/endpoints';
...
function PrintHeadingView() {
  ...
  const [user, setUser] = useState<PrintHeadingDto>({} as PrintHeadingDto);
  const [selectedUserItems, setSelectedUserItems] = useState<PrintHeadingDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(PrintHeadingDtoModel, {
    onSubmit: async (userE) => {
      await PrintHeadingDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Print Heading')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: PrintHeadingDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: PrintHeadingDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          PrintHeadingDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={PrintHeadingDtoCrudService}
              model={PrintHeadingDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'printHeading', 'description', 'idx']}
...
export default PrintHeadingView;
