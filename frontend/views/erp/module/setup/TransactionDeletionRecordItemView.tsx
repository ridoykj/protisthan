...
import TransactionDeletionRecordItemDto from 'Frontend/generated/com/itbd/application/db/dto/transactions/TransactionDeletionRecordItemDto';
import TransactionDeletionRecordItemDtoModel from 'Frontend/generated/com/itbd/application/db/dto/transactions/TransactionDeletionRecordItemDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, TransactionDeletionRecordItemDtoCrudService } from 'Frontend/generated/endpoints';
...
function TransactionDeletionRecordItemView() {
  ...
  const [user, setUser] = useState<TransactionDeletionRecordItemDto>({} as TransactionDeletionRecordItemDto);
  const [selectedUserItems, setSelectedUserItems] = useState<TransactionDeletionRecordItemDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(TransactionDeletionRecordItemDtoModel, {
    onSubmit: async (userE) => {
      await TransactionDeletionRecordItemDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Transaction Deletion Record Item')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as TransactionDeletionRecordItemDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: TransactionDeletionRecordItemDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/transaction-deletion-record-item/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: TransactionDeletionRecordItemDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          TransactionDeletionRecordItemDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={TransactionDeletionRecordItemDtoCrudService}
              model={TransactionDeletionRecordItemDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'doctypeName', 'noOfDocs', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/transaction-deletion-record-item/_`);
        }}
      >
...
export default TransactionDeletionRecordItemView;
