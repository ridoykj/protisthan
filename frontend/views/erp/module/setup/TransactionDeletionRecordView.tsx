...
import TransactionDeletionRecordDto from 'Frontend/generated/com/itbd/application/db/dto/transactions/TransactionDeletionRecordDto';
import TransactionDeletionRecordDtoModel from 'Frontend/generated/com/itbd/application/db/dto/transactions/TransactionDeletionRecordDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, TransactionDeletionRecordDtoCrudService } from 'Frontend/generated/endpoints';
...
function TransactionDeletionRecordView() {
  ...
  const [user, setUser] = useState<TransactionDeletionRecordDto>({} as TransactionDeletionRecordDto);
  const [selectedUserItems, setSelectedUserItems] = useState<TransactionDeletionRecordDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(TransactionDeletionRecordDtoModel, {
    onSubmit: async (userE) => {
      await TransactionDeletionRecordDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Transaction Deletion Record')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as TransactionDeletionRecordDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: TransactionDeletionRecordDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/transaction-deletion-record/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: TransactionDeletionRecordDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          TransactionDeletionRecordDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={TransactionDeletionRecordDtoCrudService}
              model={TransactionDeletionRecordDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'company', 'status', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/transaction-deletion-record/_`);
        }}
      >
...
export default TransactionDeletionRecordView;
