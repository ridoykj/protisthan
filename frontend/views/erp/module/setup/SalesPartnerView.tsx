...
import SalesPartnerDto from 'Frontend/generated/com/itbd/application/db/dto/sales/SalesPartnerDto';
import SalesPartnerDtoModel from 'Frontend/generated/com/itbd/application/db/dto/sales/SalesPartnerDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, SalesPartnerDtoCrudService } from 'Frontend/generated/endpoints';
...
function SalesPartnerView() {
  ...
  const [user, setUser] = useState<SalesPartnerDto>({} as SalesPartnerDto);
  const [selectedUserItems, setSelectedUserItems] = useState<SalesPartnerDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(SalesPartnerDtoModel, {
    onSubmit: async (userE) => {
      await SalesPartnerDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: SalesPartnerDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: SalesPartnerDto }) {
    const { name } = item;
    ...
    return (
      <button
        ...
        title="Delete"
        onClick={(e) => {
          SalesPartnerDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'sales partner')).then(
      (result) => {
        setUiField(result);
      }
    );
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={SalesPartnerDtoCrudService}
              model={SalesPartnerDtoModel}
              ref={autoGridRef}
...
export default SalesPartnerView;
