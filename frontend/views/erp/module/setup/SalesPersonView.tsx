...
import SalesPersonDto from 'Frontend/generated/com/itbd/application/db/dto/sales/SalesPersonDto';
import SalesPersonDtoModel from 'Frontend/generated/com/itbd/application/db/dto/sales/SalesPersonDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, SalesPersonDtoCrudService } from 'Frontend/generated/endpoints';
...
function SalesPersonView() {
  ...
  const [user, setUser] = useState<SalesPersonDto>({} as SalesPersonDto);
  const [selectedUserItems, setSelectedUserItems] = useState<SalesPersonDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(SalesPersonDtoModel, {
    onSubmit: async (userE) => {
      await SalesPersonDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: SalesPersonDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: SalesPersonDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          SalesPersonDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'sales person')).then(
      (result) => {
        setUiField(result);
      }
    );
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={SalesPersonDtoCrudService}
              model={SalesPersonDtoModel}
              ref={autoGridRef}
