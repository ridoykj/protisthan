...
import IncotermDto from 'Frontend/generated/com/itbd/application/db/dto/IncotermDto';
import IncotermDtoModel from 'Frontend/generated/com/itbd/application/db/dto/IncotermDtoModel';
...
import { DocFieldDtoCrudService, IncotermDtoCrudService } from 'Frontend/generated/endpoints';
...
function IncotermView() {
  ...
  const [user, setUser] = useState<IncotermDto>({} as IncotermDto);
  const [selectedUserItems, setSelectedUserItems] = useState<IncotermDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(IncotermDtoModel, {
    onSubmit: async (userE) => {
      await IncotermDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Incoterm')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: IncotermDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: IncotermDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          IncotermDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={IncotermDtoCrudService}
              model={IncotermDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'code', 'title', 'idx']}
