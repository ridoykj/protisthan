...
import SupplierGroupDto from 'Frontend/generated/com/itbd/application/db/dto/supplier/SupplierGroupDto';
import SupplierGroupDtoModel from 'Frontend/generated/com/itbd/application/db/dto/supplier/SupplierGroupDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, SupplierGroupDtoCrudService } from 'Frontend/generated/endpoints';
...
function SupplierGroupView() {
  ...
  const [user, setUser] = useState<SupplierGroupDto>({} as SupplierGroupDto);
  const [selectedUserItems, setSelectedUserItems] = useState<SupplierGroupDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(SupplierGroupDtoModel, {
    onSubmit: async (userE) => {
      await SupplierGroupDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Supplier Group')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: SupplierGroupDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/supplier-group/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: SupplierGroupDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          SupplierGroupDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={SupplierGroupDtoCrudService}
              model={SupplierGroupDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'supplierGroupName', 'parentSupplierGroup', 'isGroup', 'lft', 'rgt', 'idx']}
...
export default SupplierGroupView;
