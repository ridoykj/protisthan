...
import DepartmentDto from 'Frontend/generated/com/itbd/application/db/dto/DepartmentDto';
import DepartmentDtoModel from 'Frontend/generated/com/itbd/application/db/dto/DepartmentDtoModel';
...
import { DepartmentDtoCrudService, DocFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function DepartmentView() {
  ...
  const [user, setUser] = useState<DepartmentDto>({} as DepartmentDto);
  const [selectedUserItems, setSelectedUserItems] = useState<DepartmentDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(DepartmentDtoModel, {
    onSubmit: async (userE) => {
      await DepartmentDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Department')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: DepartmentDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: DepartmentDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          DepartmentDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
        ...
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              ...
              visibleColumns={['name', 'departmentName', 'parentDepartment', 'company', 'isGroup', 'lft', 'rgt', 'idx']}
