...
import PartyTypeDto from 'Frontend/generated/com/itbd/application/db/dto/PartyTypeDto';
import PartyTypeDtoModel from 'Frontend/generated/com/itbd/application/db/dto/PartyTypeDtoModel';
...
import { DocFieldDtoCrudService, PartyTypeDtoCrudService } from 'Frontend/generated/endpoints';
...
function PartyTypeView() {
  ...
  const [user, setUser] = useState<PartyTypeDto>({} as PartyTypeDto);
  const [selectedUserItems, setSelectedUserItems] = useState<PartyTypeDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(PartyTypeDtoModel, {
    onSubmit: async (userE) => {
      await PartyTypeDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Party Type')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: PartyTypeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: PartyTypeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          PartyTypeDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={PartyTypeDtoCrudService}
              model={PartyTypeDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'partyType', 'accountType', 'idx']}
