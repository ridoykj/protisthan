...
import UomConversionFactorDto from 'Frontend/generated/com/itbd/application/db/dto/uoms/UomConversionFactorDto';
import UomConversionFactorDtoModel from 'Frontend/generated/com/itbd/application/db/dto/uoms/UomConversionFactorDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, UomConversionFactorDtoCrudService } from 'Frontend/generated/endpoints';
...
function UomConversionFactorView() {
  ...
  const [user, setUser] = useState<UomConversionFactorDto>({} as UomConversionFactorDto);
  const [selectedUserItems, setSelectedUserItems] = useState<UomConversionFactorDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(UomConversionFactorDtoModel, {
    onSubmit: async (userE) => {
      await UomConversionFactorDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'UOM Conversion Factor')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as UomConversionFactorDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: UomConversionFactorDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/uom-conversion-factor/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: UomConversionFactorDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          UomConversionFactorDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
        ...
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              ...
              visibleColumns={['name', 'category', 'fromUom', 'toUom', 'value', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/uom-conversion-factor/_`);
        }}
      >
...
export default UomConversionFactorView;
