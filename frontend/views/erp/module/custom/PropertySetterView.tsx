...
import PropertySetterDto from 'Frontend/generated/com/itbd/application/db/dto/PropertySetterDto';
import PropertySetterDtoModel from 'Frontend/generated/com/itbd/application/db/dto/PropertySetterDtoModel';
...
import { DocFieldDtoCrudService, PropertySetterDtoCrudService } from 'Frontend/generated/endpoints';
...
function PropertySetterView() {
  ...
  const [user, setUser] = useState<PropertySetterDto>({} as PropertySetterDto);
  const [selectedUserItems, setSelectedUserItems] = useState<PropertySetterDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(PropertySetterDtoModel, {
    onSubmit: async (userE) => {
      await PropertySetterDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Property Setter')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: PropertySetterDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/property-setter/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: PropertySetterDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          PropertySetterDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={PropertySetterDtoCrudService}
              model={PropertySetterDtoModel}
              ref={autoGridRef}
...
export default PropertySetterView;
