...
import AccountDto from 'Frontend/generated/com/itbd/application/db/dto/accounts/AccountDto';
import AccountDtoModel from 'Frontend/generated/com/itbd/application/db/dto/accounts/AccountDtoModel';
...
function CustomFieldView() {
  ...
  const [selectedUserItems, setSelectedUserItems] = useState<AccountDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(AccountDtoModel, {
    onSubmit: async (userE) => {
      await AccountDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Custom Field')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: AccountDto }) {
    const { name } = item;
    return (
      <button
        type="button"
