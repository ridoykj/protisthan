...
import CompanyDto from 'Frontend/generated/com/itbd/application/db/dto/CompanyDto';
import CompanyDtoModel from 'Frontend/generated/com/itbd/application/db/dto/CompanyDtoModel';
...
import { CompanyDtoCrudService, DocFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function CompanyView() {
  ...
  const [user, setUser] = useState<CompanyDto>({} as CompanyDto);
  const [selectedUserItems, setSelectedUserItems] = useState<CompanyDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(CompanyDtoModel, {
    onSubmit: async (userE) => {
      await CompanyDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'company')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: CompanyDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: CompanyDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          CompanyDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={CompanyDtoCrudService}
              model={CompanyDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'country', 'parentCompany', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/company/_`);
        }}
      >
        <FormLayout responsiveSteps={responsiveSteps} className="w-fit h-fit p-2">
          <TextField label="Customer Name" {...{ colspan: 1 }} {...field(model.companyName)} />
          <TextField label="Customer Name" {...{ colspan: 1 }} {...field(model.country)} />
          <TextField label="Customer Type" {...{ colspan: 1 }} {...field(model.parentCompany)} />
        </FormLayout>
      </DialogFromRC>
    </>
  );
