...
import ModuleOnboardingDto from 'Frontend/generated/com/itbd/application/db/dto/modules/ModuleOnboardingDto';
import ModuleOnboardingDtoModel from 'Frontend/generated/com/itbd/application/db/dto/modules/ModuleOnboardingDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, ModuleOnboardingDtoCrudService } from 'Frontend/generated/endpoints';
...
function ModuleOnboardingView() {
  ...
  const [user, setUser] = useState<ModuleOnboardingDto>({} as ModuleOnboardingDto);
  const [selectedUserItems, setSelectedUserItems] = useState<ModuleOnboardingDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ModuleOnboardingDtoModel, {
    onSubmit: async (userE) => {
      await ModuleOnboardingDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Module Onboarding')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as ModuleOnboardingDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: ModuleOnboardingDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/module-onboarding/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: ModuleOnboardingDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          ModuleOnboardingDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={ModuleOnboardingDtoCrudService}
              model={ModuleOnboardingDtoModel}
              ref={autoGridRef}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/module-onboarding/_`);
        }}
      >
...
export default ModuleOnboardingView;
