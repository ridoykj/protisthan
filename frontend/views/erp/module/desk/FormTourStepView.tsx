...
import FormTourStepDto from 'Frontend/generated/com/itbd/application/db/dto/FormTourStepDto';
import FormTourStepDtoModel from 'Frontend/generated/com/itbd/application/db/dto/FormTourStepDtoModel';
...
import { DocFieldDtoCrudService, FormTourStepDtoCrudService } from 'Frontend/generated/endpoints';
...
function FormTourStepView() {
  ...
  const [user, setUser] = useState<FormTourStepDto>({} as FormTourStepDto);
  const [selectedUserItems, setSelectedUserItems] = useState<FormTourStepDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(FormTourStepDtoModel, {
    onSubmit: async (userE) => {
      await FormTourStepDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Form Tour Step')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: FormTourStepDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/form-tour-step/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: FormTourStepDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          FormTourStepDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={FormTourStepDtoCrudService}
              model={FormTourStepDtoModel}
              ref={autoGridRef}
              ...
              columnOptions={{
                ...
                ondemandDescription: {
                  header: 'Popover or Modal Description',
                  resizable: true,
                },
                label: {
                  header: 'Label',
...
export default FormTourStepView;
