...
import NotificationSettingsDto from 'Frontend/generated/com/itbd/application/db/dto/notifications/NotificationSettingsDto';
import NotificationSettingsDtoModel from 'Frontend/generated/com/itbd/application/db/dto/notifications/NotificationSettingsDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, NotificationSettingsDtoCrudService } from 'Frontend/generated/endpoints';
...
function NotificationSettingsView() {
  ...
  const [user, setUser] = useState<NotificationSettingsDto>({} as NotificationSettingsDto);
  const [selectedUserItems, setSelectedUserItems] = useState<NotificationSettingsDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(NotificationSettingsDtoModel, {
    onSubmit: async (userE) => {
      await NotificationSettingsDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Notification Settings')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as NotificationSettingsDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: NotificationSettingsDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/notification-settings/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: NotificationSettingsDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          NotificationSettingsDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={NotificationSettingsDtoCrudService}
              model={NotificationSettingsDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'user', 'seen', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/notification-settings/_`);
        }}
      >
