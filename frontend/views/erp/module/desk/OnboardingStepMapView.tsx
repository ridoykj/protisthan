...
import OnboardingStepMapDto from 'Frontend/generated/com/itbd/application/db/dto/onboarding/OnboardingStepMapDto';
import OnboardingStepMapDtoModel from 'Frontend/generated/com/itbd/application/db/dto/onboarding/OnboardingStepMapDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, OnboardingStepMapDtoCrudService } from 'Frontend/generated/endpoints';
...
function OnboardingStepMapView() {
  ...
  const [user, setUser] = useState<OnboardingStepMapDto>({} as OnboardingStepMapDto);
  const [selectedUserItems, setSelectedUserItems] = useState<OnboardingStepMapDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(OnboardingStepMapDtoModel, {
    onSubmit: async (userE) => {
      await OnboardingStepMapDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Onboarding Step Map')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as OnboardingStepMapDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: OnboardingStepMapDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/onboarding-step-map/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: OnboardingStepMapDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          OnboardingStepMapDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={OnboardingStepMapDtoCrudService}
              model={OnboardingStepMapDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'step', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/onboarding-step-map/_`);
        }}
      >
...
export default OnboardingStepMapView;
