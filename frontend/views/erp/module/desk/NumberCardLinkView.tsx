...
import NumberCardLinkDto from 'Frontend/generated/com/itbd/application/db/dto/NumberCardLinkDto';
import NumberCardLinkDtoModel from 'Frontend/generated/com/itbd/application/db/dto/NumberCardLinkDtoModel';
...
import { DocFieldDtoCrudService, NumberCardLinkDtoCrudService } from 'Frontend/generated/endpoints';
...
function NumberCardLinkView() {
  ...
  const [user, setUser] = useState<NumberCardLinkDto>({} as NumberCardLinkDto);
  const [selectedUserItems, setSelectedUserItems] = useState<NumberCardLinkDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(NumberCardLinkDtoModel, {
    onSubmit: async (userE) => {
      await NumberCardLinkDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Number Card Link')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: NumberCardLinkDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/number-card-link/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: NumberCardLinkDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          NumberCardLinkDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={NumberCardLinkDtoCrudService}
              model={NumberCardLinkDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'card', 'idx']}
...
export default NumberCardLinkView;
