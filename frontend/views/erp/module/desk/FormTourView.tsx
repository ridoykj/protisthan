...
import FormTourDto from 'Frontend/generated/com/itbd/application/db/dto/FormTourDto';
import FormTourDtoModel from 'Frontend/generated/com/itbd/application/db/dto/FormTourDtoModel';
...
import { DocFieldDtoCrudService, FormTourDtoCrudService } from 'Frontend/generated/endpoints';
...
function FormTourView() {
  ...
  const [user, setUser] = useState<FormTourDto>({} as FormTourDto);
  const [selectedUserItems, setSelectedUserItems] = useState<FormTourDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(FormTourDtoModel, {
    onSubmit: async (userE) => {
      await FormTourDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Form Tour')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: FormTourDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: FormTourDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          FormTourDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={FormTourDtoCrudService}
              model={FormTourDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'title', 'referenceDoctype', 'pageRoute', 'idx']}
