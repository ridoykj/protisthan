...
import WorkspaceChartDto from 'Frontend/generated/com/itbd/application/db/dto/workspace/WorkspaceChartDto';
import WorkspaceChartDtoModel from 'Frontend/generated/com/itbd/application/db/dto/workspace/WorkspaceChartDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, WorkspaceChartDtoCrudService } from 'Frontend/generated/endpoints';
...
function WorkspaceChartView() {
  ...
  const [user, setUser] = useState<WorkspaceChartDto>({} as WorkspaceChartDto);
  const [selectedUserItems, setSelectedUserItems] = useState<WorkspaceChartDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(WorkspaceChartDtoModel, {
    onSubmit: async (userE) => {
      await WorkspaceChartDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Workspace Chart')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: WorkspaceChartDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/workspace-chart/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: WorkspaceChartDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          WorkspaceChartDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={WorkspaceChartDtoCrudService}
              model={WorkspaceChartDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'chartName', 'label', 'idx']}
...
export default WorkspaceChartView;
