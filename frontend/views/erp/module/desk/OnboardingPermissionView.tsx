...
import OnboardingPermissionDto from 'Frontend/generated/com/itbd/application/db/dto/onboarding/OnboardingPermissionDto';
import OnboardingPermissionDtoModel from 'Frontend/generated/com/itbd/application/db/dto/onboarding/OnboardingPermissionDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, OnboardingPermissionDtoCrudService } from 'Frontend/generated/endpoints';
...
function OnboardingPermissionView() {
  ...
  const [user, setUser] = useState<OnboardingPermissionDto>({} as OnboardingPermissionDto);
  const [selectedUserItems, setSelectedUserItems] = useState<OnboardingPermissionDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(OnboardingPermissionDtoModel, {
    onSubmit: async (userE) => {
      await OnboardingPermissionDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Onboarding Permission')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as OnboardingPermissionDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: OnboardingPermissionDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/onboarding-permission/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: OnboardingPermissionDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          OnboardingPermissionDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={OnboardingPermissionDtoCrudService}
              model={OnboardingPermissionDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'role', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/onboarding-permission/_`);
        }}
      >
...
export default OnboardingPermissionView;
