...
import DashboardDto from 'Frontend/generated/com/itbd/application/db/dto/dashboardchart/DashboardDto';
import DashboardDtoModel from 'Frontend/generated/com/itbd/application/db/dto/dashboardchart/DashboardDtoModel';
...
import { DashboardDtoCrudService, DocFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function DashboardView() {
  ...
  const [user, setUser] = useState<DashboardDto>({} as DashboardDto);
  const [selectedUserItems, setSelectedUserItems] = useState<DashboardDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(DashboardDtoModel, {
    onSubmit: async (userE) => {
      await DashboardDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Dashboard')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: DashboardDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: DashboardDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          DashboardDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={DashboardDtoCrudService}
              model={DashboardDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'dashboardName', 'idx']}
