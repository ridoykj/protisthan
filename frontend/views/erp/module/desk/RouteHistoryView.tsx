...
import RouteHistoryDto from 'Frontend/generated/com/itbd/application/db/dto/RouteHistoryDto';
import RouteHistoryDtoModel from 'Frontend/generated/com/itbd/application/db/dto/RouteHistoryDtoModel';
...
import { DocFieldDtoCrudService, RouteHistoryDtoCrudService } from 'Frontend/generated/endpoints';
...
function RouteHistoryView() {
  ...
  const [user, setUser] = useState<RouteHistoryDto>({} as RouteHistoryDto);
  const [selectedUserItems, setSelectedUserItems] = useState<RouteHistoryDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(RouteHistoryDtoModel, {
    onSubmit: async (userE) => {
      await RouteHistoryDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Route History')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: RouteHistoryDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: RouteHistoryDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          RouteHistoryDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={RouteHistoryDtoCrudService}
              model={RouteHistoryDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'route', 'user', 'idx']}
