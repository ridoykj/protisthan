...
import DashboardChartFieldDto from 'Frontend/generated/com/itbd/application/db/dto/dashboardchart/DashboardChartFieldDto';
import DashboardChartFieldDtoModel from 'Frontend/generated/com/itbd/application/db/dto/dashboardchart/DashboardChartFieldDtoModel';
...
import { DashboardChartFieldDtoCrudService, DocFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function DashboardChartFieldView() {
  ...
  const [user, setUser] = useState<DashboardChartFieldDto>({} as DashboardChartFieldDto);
  const [selectedUserItems, setSelectedUserItems] = useState<DashboardChartFieldDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(DashboardChartFieldDtoModel, {
    onSubmit: async (userE) => {
      await DashboardChartFieldDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Dashboard Chart Field')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as DashboardChartFieldDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: DashboardChartFieldDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/dashboard-chart-field/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: DashboardChartFieldDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          DashboardChartFieldDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={DashboardChartFieldDtoCrudService}
              model={DashboardChartFieldDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'yField', 'color', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/dashboard-chart-field/_`);
        }}
      >
...
export default DashboardChartFieldView;
