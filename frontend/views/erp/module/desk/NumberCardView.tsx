...
import NumberCardDto from 'Frontend/generated/com/itbd/application/db/dto/NumberCardDto';
import NumberCardDtoModel from 'Frontend/generated/com/itbd/application/db/dto/NumberCardDtoModel';
...
import { DocFieldDtoCrudService, NumberCardDtoCrudService } from 'Frontend/generated/endpoints';
...
function NumberCardView() {
  ...
  const [user, setUser] = useState<NumberCardDto>({} as NumberCardDto);
  const [selectedUserItems, setSelectedUserItems] = useState<NumberCardDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(NumberCardDtoModel, {
    onSubmit: async (userE) => {
      await NumberCardDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Number Card')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: NumberCardDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: NumberCardDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          NumberCardDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={NumberCardDtoCrudService}
              model={NumberCardDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'label', 'documentType', 'idx']}
