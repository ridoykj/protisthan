...
import GlobalSearchDocTypeDto from 'Frontend/generated/com/itbd/application/db/dto/GlobalSearchDocTypeDto';
import GlobalSearchDocTypeDtoModel from 'Frontend/generated/com/itbd/application/db/dto/GlobalSearchDocTypeDtoModel';
...
import { DocFieldDtoCrudService, GlobalSearchDocTypeDtoCrudService } from 'Frontend/generated/endpoints';
...
function GlobalSearchDocTypeView() {
  ...
  const [user, setUser] = useState<GlobalSearchDocTypeDto>({} as GlobalSearchDocTypeDto);
  const [selectedUserItems, setSelectedUserItems] = useState<GlobalSearchDocTypeDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(GlobalSearchDocTypeDtoModel, {
    onSubmit: async (userE) => {
      await GlobalSearchDocTypeDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Global Search DocType')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as GlobalSearchDocTypeDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: GlobalSearchDocTypeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/global-search-doctype/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: GlobalSearchDocTypeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          GlobalSearchDocTypeDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={GlobalSearchDocTypeDtoCrudService}
              model={GlobalSearchDocTypeDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'documentType', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/global-search-doctype/_`);
        }}
      >
...
export default GlobalSearchDocTypeView;
