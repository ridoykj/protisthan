...
import WorkspaceNumberCardDto from 'Frontend/generated/com/itbd/application/db/dto/workspace/WorkspaceNumberCardDto';
import WorkspaceNumberCardDtoModel from 'Frontend/generated/com/itbd/application/db/dto/workspace/WorkspaceNumberCardDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, WorkspaceNumberCardDtoCrudService } from 'Frontend/generated/endpoints';
...
function WorkspaceNumberCardView() {
  ...
  const [user, setUser] = useState<WorkspaceNumberCardDto>({} as WorkspaceNumberCardDto);
  const [selectedUserItems, setSelectedUserItems] = useState<WorkspaceNumberCardDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(WorkspaceNumberCardDtoModel, {
    onSubmit: async (userE) => {
      await WorkspaceNumberCardDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Workspace Number Card')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as WorkspaceNumberCardDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: WorkspaceNumberCardDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/workspace-number-card/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: WorkspaceNumberCardDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          WorkspaceNumberCardDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={WorkspaceNumberCardDtoCrudService}
              model={WorkspaceNumberCardDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'numberCardName', 'label', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/workspace-number-card/_`);
        }}
      >
...
export default WorkspaceNumberCardView;
