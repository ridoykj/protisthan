...
import DashboardChartDto from 'Frontend/generated/com/itbd/application/db/dto/dashboardchart/DashboardChartDto';
import DashboardChartDtoModel from 'Frontend/generated/com/itbd/application/db/dto/dashboardchart/DashboardChartDtoModel';
...
import { DashboardChartDtoCrudService, DocFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function DashboardChartSourceView() {
  ...
  const [user, setUser] = useState<DashboardChartDto>({} as DashboardChartDto);
  const [selectedUserItems, setSelectedUserItems] = useState<DashboardChartDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(DashboardChartDtoModel, {
    onSubmit: async (userE) => {
      await DashboardChartDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Dashboard Chart Source')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: DashboardChartDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/dashboard-chart-source/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: DashboardChartDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          DashboardChartDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={DashboardChartDtoCrudService}
              model={DashboardChartDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'sourceName', 'module', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/dashboard-chart-source/_`);
        }}
      >
...
export default DashboardChartSourceView;
