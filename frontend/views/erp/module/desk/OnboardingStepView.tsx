...
import OnboardingStepDto from 'Frontend/generated/com/itbd/application/db/dto/onboarding/OnboardingStepDto';
import OnboardingStepDtoModel from 'Frontend/generated/com/itbd/application/db/dto/onboarding/OnboardingStepDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, OnboardingStepDtoCrudService } from 'Frontend/generated/endpoints';
...
function OnboardingStepView() {
  ...
  const [user, setUser] = useState<OnboardingStepDto>({} as OnboardingStepDto);
  const [selectedUserItems, setSelectedUserItems] = useState<OnboardingStepDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(OnboardingStepDtoModel, {
    onSubmit: async (userE) => {
      await OnboardingStepDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Onboarding Step')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: OnboardingStepDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/onboarding-step/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: OnboardingStepDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          OnboardingStepDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={OnboardingStepDtoCrudService}
              model={OnboardingStepDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'title', 'isComplete', 'isSkipped', 'idx']}
