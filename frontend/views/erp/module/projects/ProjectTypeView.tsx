...
import ProjectTypeDto from 'Frontend/generated/com/itbd/application/db/dto/projects/ProjectTypeDto';
import ProjectTypeDtoModel from 'Frontend/generated/com/itbd/application/db/dto/projects/ProjectTypeDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, ProjectTypeDtoCrudService } from 'Frontend/generated/endpoints';
...
function ProjectTypeView() {
  ...
  const [user, setUser] = useState<ProjectTypeDto>({} as ProjectTypeDto);
  const [selectedUserItems, setSelectedUserItems] = useState<ProjectTypeDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ProjectTypeDtoModel, {
    onSubmit: async (userE) => {
      await ProjectTypeDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Project Type')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: ProjectTypeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function DeleteRander({ item }: { item: ProjectTypeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          ProjectTypeDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={ProjectTypeDtoCrudService}
              model={ProjectTypeDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'projectType', 'idx']}
