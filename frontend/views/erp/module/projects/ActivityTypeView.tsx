...
import ActivityTypeDto from 'Frontend/generated/com/itbd/application/db/dto/activitys/ActivityTypeDto';
import ActivityTypeDtoModel from 'Frontend/generated/com/itbd/application/db/dto/activitys/ActivityTypeDtoModel';
...
import { ActivityTypeDtoCrudService, DocFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function ActivityTypeView() {
  ...
  const [user, setUser] = useState<ActivityTypeDto>({} as ActivityTypeDto);
  const [selectedUserItems, setSelectedUserItems] = useState<ActivityTypeDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ActivityTypeDtoModel, {
    onSubmit: async (userE) => {
      await ActivityTypeDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Activity Type')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: ActivityTypeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: ActivityTypeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          ActivityTypeDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={ActivityTypeDtoCrudService}
              model={ActivityTypeDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'activityType', 'idx']}
