...
import PrintFormatDto from 'Frontend/generated/com/itbd/application/db/dto/print/PrintFormatDto';
import PrintFormatDtoModel from 'Frontend/generated/com/itbd/application/db/dto/print/PrintFormatDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, PrintFormatDtoCrudService } from 'Frontend/generated/endpoints';
...
function PrintFormatView() {
  ...
  const [user, setUser] = useState<PrintFormatDto>({} as PrintFormatDto);
  const [selectedUserItems, setSelectedUserItems] = useState<PrintFormatDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(PrintFormatDtoModel, {
    onSubmit: async (userE) => {
      await PrintFormatDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Print Format')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: PrintFormatDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: PrintFormatDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          PrintFormatDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={PrintFormatDtoCrudService}
              model={PrintFormatDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'docType', 'standard', 'fontSize', 'formatData', 'printFormatBuilder', 'idx']}
...
export default PrintFormatView;
