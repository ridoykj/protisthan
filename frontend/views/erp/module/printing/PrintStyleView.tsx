...
import PrintStyleDto from 'Frontend/generated/com/itbd/application/db/dto/print/PrintStyleDto';
import PrintStyleDtoModel from 'Frontend/generated/com/itbd/application/db/dto/print/PrintStyleDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, PrintStyleDtoCrudService } from 'Frontend/generated/endpoints';
...
function PrintStyleView() {
  ...
  const [user, setUser] = useState<PrintStyleDto>({} as PrintStyleDto);
  const [selectedUserItems, setSelectedUserItems] = useState<PrintStyleDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(PrintStyleDtoModel, {
    onSubmit: async (userE) => {
      await PrintStyleDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Print Style')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: PrintStyleDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: PrintStyleDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          PrintStyleDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={PrintStyleDtoCrudService}
              model={PrintStyleDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'printStyleName', 'disabled', 'preview', 'idx']}
