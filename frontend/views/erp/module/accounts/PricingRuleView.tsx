...
import PricingRuleDto from 'Frontend/generated/com/itbd/application/db/dto/price/PricingRuleDto';
import PricingRuleDtoModel from 'Frontend/generated/com/itbd/application/db/dto/price/PricingRuleDtoModel';
...
function PricingRuleView() {
  ...
  const [user, setUser] = useState<PricingRuleDto>({} as PricingRuleDto);
  const [selectedUserItems, setSelectedUserItems] = useState<PricingRuleDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(PricingRuleDtoModel, {
    onSubmit: async (itemE) => {
      await PricingRuleDtoCrudService.save(itemE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Pricing Rule')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: PricingRuleDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: PricingRuleDto }) {
    const { name } = item;
    ...
    return (
      <button
        ...
        title="Delete"
        onClick={(e) => {
          PricingRuleDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={PricingRuleDtoCrudService}
              model={PricingRuleDtoModel}
              ref={autoGridRef}
