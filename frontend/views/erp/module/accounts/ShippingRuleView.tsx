...
import ShippingRuleDto from 'Frontend/generated/com/itbd/application/db/dto/shipments/ShippingRuleDto';
import ShippingRuleDtoModel from 'Frontend/generated/com/itbd/application/db/dto/shipments/ShippingRuleDtoModel';
...
function ShippingRuleView() {
  ...
  const [user, setUser] = useState<ShippingRuleDto>({} as ShippingRuleDto);
  const [selectedUserItems, setSelectedUserItems] = useState<ShippingRuleDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ShippingRuleDtoModel, {
    onSubmit: async (itemE) => {
      await ShippingRuleDtoCrudService.save(itemE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Shipping Rule')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: ShippingRuleDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: ShippingRuleDto }) {
    const { name } = item;
    ...
    return (
      <button
        ...
        title="Delete"
        onClick={(e) => {
          ShippingRuleDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={ShippingRuleDtoCrudService}
              model={ShippingRuleDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'label', 'calculateBasedOn', 'creation', 'idx']}
...
export default ShippingRuleView;
