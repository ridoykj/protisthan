...
import LoyaltyPointEntryDto from 'Frontend/generated/com/itbd/application/db/dto/loyaltys/LoyaltyPointEntryDto';
import LoyaltyPointEntryDtoModel from 'Frontend/generated/com/itbd/application/db/dto/loyaltys/LoyaltyPointEntryDtoModel';
...
import {
  ...
  LoyaltyPointEntryDtoCrudService,
  UomDtoCrudService,
...
function LoyaltyPointEntryView() {
  ...
  const [user, setUser] = useState<LoyaltyPointEntryDto>({} as LoyaltyPointEntryDto);
  const [selectedUserItems, setSelectedUserItems] = useState<LoyaltyPointEntryDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(LoyaltyPointEntryDtoModel, {
    onSubmit: async (itemE) => {
      await LoyaltyPointEntryDtoCrudService.save(itemE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: LoyaltyPointEntryDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/loyalty-point-entry/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: LoyaltyPointEntryDto }) {
    const { name } = item;
    ...
    return (
      <button
        ...
        title="Delete"
        onClick={(e) => {
          LoyaltyPointEntryDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'loyalty point entry')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    ...
    // },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as LoyaltyPointEntryDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={LoyaltyPointEntryDtoCrudService}
              model={LoyaltyPointEntryDtoModel}
              ref={autoGridRef}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/loyalty-point-entry/_`);
        }}
      >
...
export default LoyaltyPointEntryView;
