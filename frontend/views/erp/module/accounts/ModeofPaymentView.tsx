...
import ModeOfPaymentDto from 'Frontend/generated/com/itbd/application/db/dto/ModeOfPaymentDto';
import ModeOfPaymentDtoModel from 'Frontend/generated/com/itbd/application/db/dto/ModeOfPaymentDtoModel';
...
import { DocFieldDtoCrudService, ModeOfPaymentDtoCrudService } from 'Frontend/generated/endpoints';
...
function ModeofPaymentView() {
  ...
  const [user, setUser] = useState<ModeOfPaymentDto>({} as ModeOfPaymentDto);
  const [selectedUserItems, setSelectedUserItems] = useState<ModeOfPaymentDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ModeOfPaymentDtoModel, {
    onSubmit: async (userE) => {
      await ModeOfPaymentDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Mode of Payment')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: ModeOfPaymentDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/mode-of-payment/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: ModeOfPaymentDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          ModeOfPaymentDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={ModeOfPaymentDtoCrudService}
              model={ModeOfPaymentDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'modeOfPayment', 'type', 'idx']}
...
export default ModeofPaymentView;
