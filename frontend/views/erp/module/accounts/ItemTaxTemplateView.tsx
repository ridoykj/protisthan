...
import ItemTaxTemplateDto from 'Frontend/generated/com/itbd/application/db/dto/items/ItemTaxTemplateDto';
import ItemTaxTemplateDtoModel from 'Frontend/generated/com/itbd/application/db/dto/items/ItemTaxTemplateDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import {
  DocFieldDtoCrudService,
  ItemTaxTemplateDtoCrudService,
...
function ItemTaxTemplateView() {
  ...
  const [user, setUser] = useState<ItemTaxTemplateDto>({} as ItemTaxTemplateDto);
  const [selectedUserItems, setSelectedUserItems] = useState<ItemTaxTemplateDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ItemTaxTemplateDtoModel, {
    onSubmit: async (userE) => {
      await ItemTaxTemplateDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Item Tax Template')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: ItemTaxTemplateDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/item-tax-template/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: ItemTaxTemplateDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          ItemTaxTemplateDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={ItemTaxTemplateDtoCrudService}
              model={ItemTaxTemplateDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'title', 'company', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/item-tax-template/_`);
        }}
      >
...
export default ItemTaxTemplateView;
