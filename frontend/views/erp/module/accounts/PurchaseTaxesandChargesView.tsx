...
import PurchaseTaxesAndChargesDto from 'Frontend/generated/com/itbd/application/db/dto/purchases/PurchaseTaxesAndChargesDto';
import PurchaseTaxesAndChargesDtoModel from 'Frontend/generated/com/itbd/application/db/dto/purchases/PurchaseTaxesAndChargesDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import {
  DocFieldDtoCrudService,
  PurchaseTaxesAndChargesDtoCrudService,
...
function PurchaseTaxesandChargesView() {
  ...
  const [user, setUser] = useState<PurchaseTaxesAndChargesDto>({} as PurchaseTaxesAndChargesDto);
  const [selectedUserItems, setSelectedUserItems] = useState<PurchaseTaxesAndChargesDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(PurchaseTaxesAndChargesDtoModel, {
    onSubmit: async (userE) => {
      await PurchaseTaxesAndChargesDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Purchase Taxes and Charges')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as PurchaseTaxesAndChargesDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: PurchaseTaxesAndChargesDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/purchase-taxes-and-charges/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: PurchaseTaxesAndChargesDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          PurchaseTaxesAndChargesDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={PurchaseTaxesAndChargesDtoCrudService}
              model={PurchaseTaxesAndChargesDtoModel}
              ref={autoGridRef}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/purchase-taxes-and-charges/_`);
        }}
      >
...
export default PurchaseTaxesandChargesView;
