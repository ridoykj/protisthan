...
import CurrencyExchangeSettingsDetailsDto from 'Frontend/generated/com/itbd/application/db/dto/currencys/CurrencyExchangeSettingsDetailsDto';
import CurrencyExchangeSettingsDetailsDtoModel from 'Frontend/generated/com/itbd/application/db/dto/currencys/CurrencyExchangeSettingsDetailsDtoModel';
...
import {
  CurrencyExchangeSettingsDetailsDtoCrudService,
...
function CurrencyExchangeSettingsDetailsView() {
  ...
  const [user, setUser] = useState<CurrencyExchangeSettingsDetailsDto>(
    {} as CurrencyExchangeSettingsDetailsDto
  );
  const [selectedUserItems, setSelectedUserItems] = useState<CurrencyExchangeSettingsDetailsDto[]>(
    []
  );
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(CurrencyExchangeSettingsDetailsDtoModel, {
    onSubmit: async (userE) => {
      await CurrencyExchangeSettingsDetailsDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Currency Exchange Settings Details')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as CurrencyExchangeSettingsDetailsDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: CurrencyExchangeSettingsDetailsDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/currency-exchange-settings-details/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: CurrencyExchangeSettingsDetailsDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          CurrencyExchangeSettingsDetailsDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={CurrencyExchangeSettingsDetailsDtoCrudService}
              model={CurrencyExchangeSettingsDetailsDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'key', 'value', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/currency-exchange-settings-details/_`);
        }}
      >
...
export default CurrencyExchangeSettingsDetailsView;
