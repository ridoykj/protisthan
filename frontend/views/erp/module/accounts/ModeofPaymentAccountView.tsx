...
import ModeOfPaymentAccountDto from 'Frontend/generated/com/itbd/application/db/dto/ModeOfPaymentAccountDto';
import ModeOfPaymentAccountDtoModel from 'Frontend/generated/com/itbd/application/db/dto/ModeOfPaymentAccountDtoModel';
...
import {
  ...
  ModeOfPaymentAccountDtoCrudService,
...
function ModeofPaymentAccountView() {
  ...
  const [user, setUser] = useState<ModeOfPaymentAccountDto>({} as ModeOfPaymentAccountDto);
  const [selectedUserItems, setSelectedUserItems] = useState<ModeOfPaymentAccountDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ModeOfPaymentAccountDtoModel, {
    onSubmit: async (userE) => {
      await ModeOfPaymentAccountDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Mode of Payment Account')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as ModeOfPaymentAccountDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: ModeOfPaymentAccountDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/mode-of-payment-account/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: ModeOfPaymentAccountDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          ModeOfPaymentAccountDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={ModeOfPaymentAccountDtoCrudService}
              model={ModeOfPaymentAccountDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'company', 'defaultAccount', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/mode-of-payment-account/_`);
        }}
      >
...
export default ModeofPaymentAccountView;
