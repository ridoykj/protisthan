...
import PosClosingEntryDto from 'Frontend/generated/com/itbd/application/db/dto/pos/PosClosingEntryDto';
import PosClosingEntryDtoModel from 'Frontend/generated/com/itbd/application/db/dto/pos/PosClosingEntryDtoModel';
...
function PosClosingEntryView() {
  ...
  const [user, setUser] = useState<PosClosingEntryDto>({} as PosClosingEntryDto);
  const [selectedUserItems, setSelectedUserItems] = useState<PosClosingEntryDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(PosClosingEntryDtoModel, {
    onSubmit: async (itemE) => {
      await PosClosingEntryDtoCrudService.save(itemE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: PosClosingEntryDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/pos-closing-entry/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: PosClosingEntryDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          PosClosingEntryDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'pos closing entry')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={PosClosingEntryDtoCrudService}
              model={PosClosingEntryDtoModel}
              ref={autoGridRef}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/pos-closing-entry/_`);
        }}
      >
...
export default PosClosingEntryView;
