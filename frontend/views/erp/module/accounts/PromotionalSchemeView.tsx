...
import PromotionalSchemeDto from 'Frontend/generated/com/itbd/application/db/dto/PromotionalSchemeDto';
import PromotionalSchemeDtoModel from 'Frontend/generated/com/itbd/application/db/dto/PromotionalSchemeDtoModel';
...
import {
  ...
  PromotionalSchemeDtoCrudService,
  UomDtoCrudService,
...
function PromotionalSchemeView() {
  ...
  const [user, setUser] = useState<PromotionalSchemeDto>({} as PromotionalSchemeDto);
  const [selectedUserItems, setSelectedUserItems] = useState<PromotionalSchemeDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(PromotionalSchemeDtoModel, {
    onSubmit: async (itemE) => {
      await PromotionalSchemeDtoCrudService.save(itemE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Promotional Scheme')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    ...
    // },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as PromotionalSchemeDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: PromotionalSchemeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/promotional-scheme/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: PromotionalSchemeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          PromotionalSchemeDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={PromotionalSchemeDtoCrudService}
              model={PromotionalSchemeDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'applyOn', 'company', 'creation', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/promotional-scheme/_`);
        }}
      >
...
export default PromotionalSchemeView;
