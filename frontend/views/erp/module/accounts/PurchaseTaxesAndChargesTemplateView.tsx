...
import PurchaseTaxesAndChargesTemplateDto from 'Frontend/generated/com/itbd/application/db/dto/purchases/PurchaseTaxesAndChargesTemplateDto';
import PurchaseTaxesAndChargesTemplateDtoModel from 'Frontend/generated/com/itbd/application/db/dto/purchases/PurchaseTaxesAndChargesTemplateDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import {
  DocFieldDtoCrudService,
  PurchaseTaxesAndChargesTemplateDtoCrudService,
...
function PurchaseTaxesAndChargesTemplateView() {
  ...
  const [user, setUser] = useState<PurchaseTaxesAndChargesTemplateDto>(
    {} as PurchaseTaxesAndChargesTemplateDto
  );
  const [selectedUserItems, setSelectedUserItems] = useState<PurchaseTaxesAndChargesTemplateDto[]>(
    []
  );
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(PurchaseTaxesAndChargesTemplateDtoModel, {
    onSubmit: async (userE) => {
      await PurchaseTaxesAndChargesTemplateDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Purchase Taxes and Charges Template')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as PurchaseTaxesAndChargesTemplateDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: PurchaseTaxesAndChargesTemplateDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/purchase-taxes-and-charges-template/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: PurchaseTaxesAndChargesTemplateDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          PurchaseTaxesAndChargesTemplateDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={PurchaseTaxesAndChargesTemplateDtoCrudService}
              model={PurchaseTaxesAndChargesTemplateDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'isDefault', 'disabled', 'company', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/purchase-taxes-and-charges-template/_`);
        }}
      >
...
export default PurchaseTaxesAndChargesTemplateView;
