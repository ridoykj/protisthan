...
import PosProfileDto from 'Frontend/generated/com/itbd/application/db/dto/pos/PosProfileDto';
import PosProfileDtoModel from 'Frontend/generated/com/itbd/application/db/dto/pos/PosProfileDtoModel';
...
function PosProfileView() {
  ...
  const [user, setUser] = useState<PosProfileDto>({} as PosProfileDto);
  const [selectedUserItems, setSelectedUserItems] = useState<PosProfileDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(PosProfileDtoModel, {
    onSubmit: async (itemE) => {
      await PosProfileDtoCrudService.save(itemE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: PosProfileDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: PosProfileDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          PosProfileDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Pos Profile')).then(
      (result) => {
        setUiField(result);
      }
    );
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={PosProfileDtoCrudService}
              model={PosProfileDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'company', 'updateStock', 'creation', 'idx']}
