...
import ProcessDeferredAccountingDto from 'Frontend/generated/com/itbd/application/db/dto/ProcessDeferredAccountingDto';
import ProcessDeferredAccountingDtoModel from 'Frontend/generated/com/itbd/application/db/dto/ProcessDeferredAccountingDtoModel';
...
import {
  ...
  ProcessDeferredAccountingDtoCrudService,
...
function ProcessDeferredAccountingView() {
  ...
  const [user, setUser] = useState<ProcessDeferredAccountingDto>(
    {} as ProcessDeferredAccountingDto
  );
  const [selectedUserItems, setSelectedUserItems] = useState<ProcessDeferredAccountingDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ProcessDeferredAccountingDtoModel, {
    onSubmit: async (userE) => {
      await ProcessDeferredAccountingDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Process Deferred Accounting')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as ProcessDeferredAccountingDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: ProcessDeferredAccountingDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/process-deferred-accounting/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: ProcessDeferredAccountingDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          ProcessDeferredAccountingDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={ProcessDeferredAccountingDtoCrudService}
              model={ProcessDeferredAccountingDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'type', 'postingDate', 'startDate', 'endDate', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/process-deferred-accounting/_`);
        }}
      >
...
export default ProcessDeferredAccountingView;
