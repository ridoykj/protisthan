...
import CurrencyExchangeSettingsResultDto from 'Frontend/generated/com/itbd/application/db/dto/currencys/CurrencyExchangeSettingsResultDto';
import CurrencyExchangeSettingsResultDtoModel from 'Frontend/generated/com/itbd/application/db/dto/currencys/CurrencyExchangeSettingsResultDtoModel';
...
import {
  CurrencyExchangeSettingsResultDtoCrudService,
...
function CurrencyExchangeSettingsResultView() {
  ...
  const [user, setUser] = useState<CurrencyExchangeSettingsResultDto>(
    {} as CurrencyExchangeSettingsResultDto
  );
  const [selectedUserItems, setSelectedUserItems] = useState<CurrencyExchangeSettingsResultDto[]>(
    []
  );
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(CurrencyExchangeSettingsResultDtoModel, {
    onSubmit: async (userE) => {
      await CurrencyExchangeSettingsResultDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Currency Exchange Settings Result')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as CurrencyExchangeSettingsResultDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: CurrencyExchangeSettingsResultDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/currency-exchange-settings-result/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: CurrencyExchangeSettingsResultDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          CurrencyExchangeSettingsResultDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={CurrencyExchangeSettingsResultDtoCrudService}
              model={CurrencyExchangeSettingsResultDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'hey', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/currency-exchange-settings-result/_`);
        }}
      >
...
export default CurrencyExchangeSettingsResultView;
