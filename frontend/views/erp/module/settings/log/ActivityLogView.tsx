...
import ActivityLogDto from 'Frontend/generated/com/itbd/application/db/dto/activitys/ActivityLogDto';
import ActivityLogDtoModel from 'Frontend/generated/com/itbd/application/db/dto/activitys/ActivityLogDtoModel';
...
function ActivityLogView() {
  ...
  const [selectedUserItems, setSelectedUserItems] = useState<ActivityLogDto[]>([]);

  const [selectedRole, setSelectedRole] = useState<ActivityLogDto>({} as ActivityLogDto);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ActivityLogDtoModel, {
    onSubmit: async (roleE) => {
      await ActivityLogDtoCrudService.save(roleE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: ActivityLogDto }) {
    const { subject, name } = item;
    return (
      <button
        type="button"
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={ActivityLogDtoCrudService}
