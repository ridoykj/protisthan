...
import WorkflowActionMasterDto from 'Frontend/generated/com/itbd/application/db/dto/workflow/WorkflowActionMasterDto';
import WorkflowActionMasterDtoModel from 'Frontend/generated/com/itbd/application/db/dto/workflow/WorkflowActionMasterDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, WorkflowActionMasterDtoCrudService } from 'Frontend/generated/endpoints';
...
function WorkflowActionMasterView() {
  ...
  const [user, setUser] = useState<WorkflowActionMasterDto>({} as WorkflowActionMasterDto);
  const [selectedUserItems, setSelectedUserItems] = useState<WorkflowActionMasterDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(WorkflowActionMasterDtoModel, {
    onSubmit: async (userE) => {
      await WorkflowActionMasterDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as WorkflowActionMasterDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: WorkflowActionMasterDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/workflow-action-master/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: WorkflowActionMasterDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          WorkflowActionMasterDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={WorkflowActionMasterDtoCrudService}
              model={WorkflowActionMasterDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'workflowActionName', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/workflow-action-master/_`);
        }}
      >
...
export default WorkflowActionMasterView;
