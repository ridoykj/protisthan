...
import BlanketOrderDto from 'Frontend/generated/com/itbd/application/db/dto/BlanketOrderDto';
import BlanketOrderDtoModel from 'Frontend/generated/com/itbd/application/db/dto/BlanketOrderDtoModel';
...
import { BlanketOrderDtoCrudService, DocFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function BlanketOrderView() {
  ...
  const [user, setUser] = useState<BlanketOrderDto>({} as BlanketOrderDto);
  const [selectedUserItems, setSelectedUserItems] = useState<BlanketOrderDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(BlanketOrderDtoModel, {
    onSubmit: async (userE) => {
      await BlanketOrderDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: BlanketOrderDto }) {
  ...
  function deleteRander({ item }: { item: BlanketOrderDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          BlanketOrderDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Blanket Order')).then(
      (result) => {
        setUiField(result);
      }
    );
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={BlanketOrderDtoCrudService}
              model={BlanketOrderDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'blanketOrderType', 'creation', 'idx']}
...
export default BlanketOrderView;
