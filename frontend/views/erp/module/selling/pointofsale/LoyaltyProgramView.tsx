...
import LoyaltyProgramDto from 'Frontend/generated/com/itbd/application/db/dto/loyaltys/LoyaltyProgramDto';
import LoyaltyProgramDtoModel from 'Frontend/generated/com/itbd/application/db/dto/loyaltys/LoyaltyProgramDtoModel';
...
function LoyaltyProgramView() {
  ...
  const [user, setUser] = useState<LoyaltyProgramDto>({} as LoyaltyProgramDto);
  const [selectedUserItems, setSelectedUserItems] = useState<LoyaltyProgramDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(LoyaltyProgramDtoModel, {
    onSubmit: async (itemE) => {
      await LoyaltyProgramDtoCrudService.save(itemE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: LoyaltyProgramDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/loyalty-program/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: LoyaltyProgramDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          LoyaltyProgramDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'loyalty program')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={LoyaltyProgramDtoCrudService}
              model={LoyaltyProgramDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'loyaltyProgramName', 'creation', 'idx']}
...
export default LoyaltyProgramView;
