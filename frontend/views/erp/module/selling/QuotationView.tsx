...
import QuotationDto from 'Frontend/generated/com/itbd/application/db/dto/quotations/QuotationDto';
import QuotationDtoModel from 'Frontend/generated/com/itbd/application/db/dto/quotations/QuotationDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, QuotationDtoCrudService } from 'Frontend/generated/endpoints';
...
function QuotationView() {
  ...
  const [user, setUser] = useState<QuotationDto>({} as QuotationDto);
  const [selectedUserItems, setSelectedUserItems] = useState<QuotationDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(QuotationDtoModel, {
    onSubmit: async (userE) => {
      await QuotationDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: QuotationDto }) {
  ...
  function deleteRander({ item }: { item: QuotationDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          QuotationDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Quotation')).then(
      (result) => {
        setUiField(result);
      }
    );
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={QuotationDtoCrudService}
              model={QuotationDtoModel}
              ref={autoGridRef}
