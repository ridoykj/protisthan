...
import ProductBundleDto from 'Frontend/generated/com/itbd/application/db/dto/ProductBundleDto';
import ProductBundleDtoModel from 'Frontend/generated/com/itbd/application/db/dto/ProductBundleDtoModel';
...
function ProductBundleView() {
  ...
  const [user, setUser] = useState<ProductBundleDto>({} as ProductBundleDto);
  const [selectedUserItems, setSelectedUserItems] = useState<ProductBundleDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ProductBundleDtoModel, {
    onSubmit: async (itemE) => {
      await ProductBundleDtoCrudService.save(itemE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Product bundle')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: ProductBundleDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/product-bundle/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: ProductBundleDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          ProductBundleDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={ProductBundleDtoCrudService}
              model={ProductBundleDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'newItemCode', 'description', 'creation', 'idx']}
