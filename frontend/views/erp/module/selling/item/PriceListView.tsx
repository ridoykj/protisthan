...
import PriceListDto from 'Frontend/generated/com/itbd/application/db/dto/price/PriceListDto';
import PriceListDtoModel from 'Frontend/generated/com/itbd/application/db/dto/price/PriceListDtoModel';
...
function PriceListView() {
  ...
  const [user, setUser] = useState<PriceListDto>({} as PriceListDto);
  const [selectedUserItems, setSelectedUserItems] = useState<PriceListDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(PriceListDtoModel, {
    onSubmit: async (itemE) => {
      await PriceListDtoCrudService.save(itemE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'price list')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: PriceListDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: PriceListDto }) {
    const { name } = item;
    ...
    return (
      <button
        ...
        title="Delete"
        onClick={(e) => {
          PriceListDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={PriceListDtoCrudService}
              model={PriceListDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'priceListName', 'currency', 'buying', 'selling', 'idx']}
