...
import IndustryTypeDto from 'Frontend/generated/com/itbd/application/db/dto/IndustryTypeDto';
import IndustryTypeDtoModel from 'Frontend/generated/com/itbd/application/db/dto/IndustryTypeDtoModel';
...
import { DocFieldDtoCrudService, IndustryTypeDtoCrudService } from 'Frontend/generated/endpoints';
...
function IndustryTypeView() {
  ...
  const [user, setUser] = useState<IndustryTypeDto>({} as IndustryTypeDto);
  const [selectedUserItems, setSelectedUserItems] = useState<IndustryTypeDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(IndustryTypeDtoModel, {
    onSubmit: async (userE) => {
      await IndustryTypeDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Industry Type')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: IndustryTypeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: IndustryTypeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          IndustryTypeDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={IndustryTypeDtoCrudService}
              model={IndustryTypeDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'industry', 'idx']}
...
export default IndustryTypeView;
