...
import CampaignDto from 'Frontend/generated/com/itbd/application/db/dto/campaign/CampaignDto';
import CampaignDtoModel from 'Frontend/generated/com/itbd/application/db/dto/campaign/CampaignDtoModel';
...
import { CampaignDtoCrudService, DocFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function CampaignView() {
  ...
  const [user, setUser] = useState<CampaignDto>({} as CampaignDto);
  const [selectedUserItems, setSelectedUserItems] = useState<CampaignDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(CampaignDtoModel, {
    onSubmit: async (userE) => {
      await CampaignDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: CampaignDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: CampaignDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          CampaignDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Campaign')).then(
      (result) => {
        setUiField(result);
      }
    );
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={CampaignDtoCrudService}
              model={CampaignDtoModel}
              ref={autoGridRef}
