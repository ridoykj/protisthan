...
import SalesOrderDto from 'Frontend/generated/com/itbd/application/db/dto/sales/SalesOrderDto';
import SalesOrderDtoModel from 'Frontend/generated/com/itbd/application/db/dto/sales/SalesOrderDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, SalesOrderDtoCrudService } from 'Frontend/generated/endpoints';
...
function SalesOrderView() {
  ...
  const [user, setUser] = useState<SalesOrderDto>({} as SalesOrderDto);
  const [selectedUserItems, setSelectedUserItems] = useState<SalesOrderDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(SalesOrderDtoModel, {
    onSubmit: async (userE) => {
      await SalesOrderDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: SalesOrderDto }) {
  ...
  function deleteRander({ item }: { item: SalesOrderDto }) {
    const { name } = item;
    ...
    return (
      <button
        ...
        title="Delete"
        onClick={(e) => {
          SalesOrderDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'sales Order')).then(
      (result) => {
        setUiField(result);
      }
    );
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={SalesOrderDtoCrudService}
              model={SalesOrderDtoModel}
              ref={autoGridRef}
