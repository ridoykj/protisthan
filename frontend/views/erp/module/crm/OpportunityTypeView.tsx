...
import OpportunityTypeDto from 'Frontend/generated/com/itbd/application/db/dto/opportunity/OpportunityTypeDto';
import OpportunityTypeDtoModel from 'Frontend/generated/com/itbd/application/db/dto/opportunity/OpportunityTypeDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import {
  DocFieldDtoCrudService,
  OpportunityTypeDtoCrudService,
...
function OpportunityTypeView() {
  ...
  const [user, setUser] = useState<OpportunityTypeDto>({} as OpportunityTypeDto);
  const [selectedUserItems, setSelectedUserItems] = useState<OpportunityTypeDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(OpportunityTypeDtoModel, {
    onSubmit: async (userE) => {
      await OpportunityTypeDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Opportunity Type')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: OpportunityTypeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/opportunity-type/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: OpportunityTypeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          OpportunityTypeDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={OpportunityTypeDtoCrudService}
...
export default OpportunityTypeView;
