...
import SalesStageDto from 'Frontend/generated/com/itbd/application/db/dto/sales/SalesStageDto';
import SalesStageDtoModel from 'Frontend/generated/com/itbd/application/db/dto/sales/SalesStageDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, SalesStageDtoCrudService } from 'Frontend/generated/endpoints';
...
function SalesStageView() {
  ...
  const [user, setUser] = useState<SalesStageDto>({} as SalesStageDto);
  const [selectedUserItems, setSelectedUserItems] = useState<SalesStageDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(SalesStageDtoModel, {
    onSubmit: async (userE) => {
      await SalesStageDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Sales Stage')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: SalesStageDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: SalesStageDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          SalesStageDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={SalesStageDtoCrudService}
