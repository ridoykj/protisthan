...
import CustomerGroupDto from 'Frontend/generated/com/itbd/application/db/dto/customers/CustomerGroupDto';
import CustomerGroupDtoModel from 'Frontend/generated/com/itbd/application/db/dto/customers/CustomerGroupDtoModel';
...
function CustomerGroupView() {
  ...
  const [user, setUser] = useState<CustomerGroupDto>({} as CustomerGroupDto);
  const [selectedUserItems, setSelectedUserItems] = useState<CustomerGroupDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(CustomerGroupDtoModel, {
    onSubmit: async (userE) => {
      await CustomerGroupDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Customer Group')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: CustomerGroupDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/customer-group/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: CustomerGroupDto }) {
    return (
      <button
        type="button"
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={CustomerGroupDtoCrudService}
              model={CustomerGroupDtoModel}
              ref={autoGridRef}
