...
import MarketSegmentDto from 'Frontend/generated/com/itbd/application/db/dto/MarketSegmentDto';
import MarketSegmentDtoModel from 'Frontend/generated/com/itbd/application/db/dto/MarketSegmentDtoModel';
...
import { DocFieldDtoCrudService, MarketSegmentDtoCrudService } from 'Frontend/generated/endpoints';
...
function MarketSegmentView() {
  ...
  const [user, setUser] = useState<MarketSegmentDto>({} as MarketSegmentDto);
  const [selectedUserItems, setSelectedUserItems] = useState<MarketSegmentDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(MarketSegmentDtoModel, {
    onSubmit: async (userE) => {
      await MarketSegmentDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Market Segment')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: MarketSegmentDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/market-segment/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: MarketSegmentDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          MarketSegmentDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={MarketSegmentDtoCrudService}
