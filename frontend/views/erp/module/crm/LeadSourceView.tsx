...
import LeadSourceDto from 'Frontend/generated/com/itbd/application/db/dto/LeadSourceDto';
import LeadSourceDtoModel from 'Frontend/generated/com/itbd/application/db/dto/LeadSourceDtoModel';
...
import { DocFieldDtoCrudService, LeadSourceDtoCrudService } from 'Frontend/generated/endpoints';
...
function LeadSourceView() {
  ...
  const [user, setUser] = useState<LeadSourceDto>({} as LeadSourceDto);
  const [selectedUserItems, setSelectedUserItems] = useState<LeadSourceDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(LeadSourceDtoModel, {
    onSubmit: async (userE) => {
      await LeadSourceDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  function ChildRedirect({ item }: { item: LeadSourceDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: LeadSourceDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          LeadSourceDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Lead Source')).then(
      (result) => {
        setUiField(result);
      }
    );
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={LeadSourceDtoCrudService}
              model={LeadSourceDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'sourceName', 'idx']}
