...
import LeadDto from 'Frontend/generated/com/itbd/application/db/dto/LeadDto';
import LeadDtoModel from 'Frontend/generated/com/itbd/application/db/dto/LeadDtoModel';
...
function LeadView() {
  ...
  const [user, setUser] = useState<LeadDto>({} as LeadDto);
  const [selectedUserItems, setSelectedUserItems] = useState<LeadDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(LeadDtoModel, {
    onSubmit: async (userE) => {
      await LeadDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'lead')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: LeadDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: LeadDto }) {
    return (
      <button
        type="button"
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={LeadDtoCrudService}
              model={LeadDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'jobTitle', 'status', 'title', 'territory', 'idx']}
