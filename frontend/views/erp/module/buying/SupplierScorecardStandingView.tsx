...
import SupplierScorecardStandingDto from 'Frontend/generated/com/itbd/application/db/dto/supplier/SupplierScorecardStandingDto';
import SupplierScorecardStandingDtoModel from 'Frontend/generated/com/itbd/application/db/dto/supplier/SupplierScorecardStandingDtoModel';
...
import {
  ...
  SupplierScorecardStandingDtoCrudService,
...
function SupplierScorecardStandingView() {
  ...
  const [user, setUser] = useState<SupplierScorecardStandingDto>(
    {} as SupplierScorecardStandingDto
  );
  const [selectedUserItems, setSelectedUserItems] = useState<SupplierScorecardStandingDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(SupplierScorecardStandingDtoModel, {
    onSubmit: async (userE) => {
      await SupplierScorecardStandingDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Supplier Scorecard Standing')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    ...
    // },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as SupplierScorecardStandingDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: SupplierScorecardStandingDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/supplier-scorecard-standing/${name}`);
        }}
      >
        {' '}
        <img
  ...
  function DeleteRander({ item }: { item: SupplierScorecardStandingDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          SupplierScorecardStandingDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={SupplierScorecardStandingDtoCrudService}
              model={SupplierScorecardStandingDtoModel}
              ref={autoGridRef}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/supplier-scorecard-standing/_`);
        }}
      >
...
export default SupplierScorecardStandingView;
