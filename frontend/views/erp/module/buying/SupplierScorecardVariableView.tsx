...
import SupplierScorecardVariableDto from 'Frontend/generated/com/itbd/application/db/dto/supplier/SupplierScorecardVariableDto';
import SupplierScorecardVariableDtoModel from 'Frontend/generated/com/itbd/application/db/dto/supplier/SupplierScorecardVariableDtoModel';
...
import {
  ...
  SupplierScorecardVariableDtoCrudService,
...
function SupplierScorecardVariableView() {
  ...
  const [user, setUser] = useState<SupplierScorecardVariableDto>(
    {} as SupplierScorecardVariableDto
  );
  const [selectedUserItems, setSelectedUserItems] = useState<SupplierScorecardVariableDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(SupplierScorecardVariableDtoModel, {
    onSubmit: async (userE) => {
      await SupplierScorecardVariableDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Supplier Scorecard Variable')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    ...
    // },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as SupplierScorecardVariableDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: SupplierScorecardVariableDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/supplier-scorecard-variable/${name}`);
        }}
      >
        {' '}
        <img
  ...
  function DeleteRander({ item }: { item: SupplierScorecardVariableDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          SupplierScorecardVariableDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={SupplierScorecardVariableDtoCrudService}
              model={SupplierScorecardVariableDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'variableLabel', 'paramName', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/supplier-scorecard-variable/_`);
        }}
      >
        <FormLayout responsiveSteps={responsiveSteps} className="w-fit h-fit p-2">
          ...
          <TextField
            ...
            {...{ colspan: 1 }}
            {...field(model.supplierPrimaryAddress)}
          /> */}
        </FormLayout>
      </DialogFromRC>
    </>
...
export default SupplierScorecardVariableView;
