...
import DeletedDocumentDto from 'Frontend/generated/com/itbd/application/db/dto/DeletedDocumentDto';
import DeletedDocumentDtoModel from 'Frontend/generated/com/itbd/application/db/dto/DeletedDocumentDtoModel';
...
function DeletedDocumentView() {
  ...
  const [user, setUser] = useState<DeletedDocumentDto>({} as DeletedDocumentDto);
  const [selectedUserItems, setSelectedUserItems] = useState<DeletedDocumentDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(DeletedDocumentDtoModel, {
    onSubmit: async (userE) => {
      await DeletedDocumentDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Deleted Document')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: DeletedDocumentDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/deleted-document/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: DeletedDocumentDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          DeletedDocumentDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={DeletedDocumentDtoCrudService}
              model={DeletedDocumentDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'deletedName', 'deletedDoctype', 'restored', 'idx']}
...
export default DeletedDocumentView;
