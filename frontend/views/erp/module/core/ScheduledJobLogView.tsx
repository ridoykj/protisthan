...
import ScheduledJobLogDto from 'Frontend/generated/com/itbd/application/db/dto/ScheduledJobLogDto';
import ScheduledJobLogDtoModel from 'Frontend/generated/com/itbd/application/db/dto/ScheduledJobLogDtoModel';
...
function ScheduledJobLogView() {
  ...
  const [user, setUser] = useState<ScheduledJobLogDto>({} as ScheduledJobLogDto);
  const [selectedUserItems, setSelectedUserItems] = useState<ScheduledJobLogDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ScheduledJobLogDtoModel, {
    onSubmit: async (userE) => {
      await ScheduledJobLogDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Scheduled Job Log')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: ScheduledJobLogDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/scheduled-job-log/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: ScheduledJobLogDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          ScheduledJobLogDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={ScheduledJobLogDtoCrudService}
              model={ScheduledJobLogDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'status', 'scheduledJobType', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/scheduled-job-log/_`);
        }}
      >
...
export default ScheduledJobLogView;
