...
import DomainDto from 'Frontend/generated/com/itbd/application/db/dto/DomainDto';
import DomainDtoModel from 'Frontend/generated/com/itbd/application/db/dto/DomainDtoModel';
...
import { DocFieldDtoCrudService, DomainDtoCrudService } from 'Frontend/generated/endpoints';
...
function DomainView() {
  ...
  const [user, setUser] = useState<DomainDto>({} as DomainDto);
  const [selectedUserItems, setSelectedUserItems] = useState<DomainDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(DomainDtoModel, {
    onSubmit: async (userE) => {
      await DomainDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Domain')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: DomainDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function DeleteRander({ item }: { item: DomainDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          DomainDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={DomainDtoCrudService}
              model={DomainDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'domain', 'idx']}
