...
import LogsToClearDto from 'Frontend/generated/com/itbd/application/db/dto/LogsToClearDto';
import LogsToClearDtoModel from 'Frontend/generated/com/itbd/application/db/dto/LogsToClearDtoModel';
...
import { DocFieldDtoCrudService, LogsToClearDtoCrudService } from 'Frontend/generated/endpoints';
...
function LogsToClearView() {
  ...
  const [user, setUser] = useState<LogsToClearDto>({} as LogsToClearDto);
  const [selectedUserItems, setSelectedUserItems] = useState<LogsToClearDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(LogsToClearDtoModel, {
    onSubmit: async (userE) => {
      await LogsToClearDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Logs To Clear')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: LogsToClearDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function DeleteRander({ item }: { item: LogsToClearDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          LogsToClearDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={LogsToClearDtoCrudService}
              model={LogsToClearDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'refDoctype', 'days', 'idx']}
...
export default LogsToClearView;
