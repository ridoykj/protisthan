...
import DocTypeActionDto from 'Frontend/generated/com/itbd/application/db/dto/doctypes/DocTypeActionDto';
import DocTypeActionDtoModel from 'Frontend/generated/com/itbd/application/db/dto/doctypes/DocTypeActionDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, DocTypeActionDtoCrudService } from 'Frontend/generated/endpoints';
...
function DocTypeActionView() {
  ...
  const [user, setUser] = useState<DocTypeActionDto>({} as DocTypeActionDto);
  const [selectedUserItems, setSelectedUserItems] = useState<DocTypeActionDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(DocTypeActionDtoModel, {
    onSubmit: async (userE) => {
      await DocTypeActionDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'DocType Action')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: DocTypeActionDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/doctype-action/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: DocTypeActionDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          DocTypeActionDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={DocTypeActionDtoCrudService}
              model={DocTypeActionDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'label', 'actionType', 'action', 'group', 'custom', 'idx']}
...
export default DocTypeActionView;
