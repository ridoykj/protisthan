...
import VersionDto from 'Frontend/generated/com/itbd/application/db/dto/VersionDto';
import VersionDtoModel from 'Frontend/generated/com/itbd/application/db/dto/VersionDtoModel';
...
import { DocFieldDtoCrudService, VersionDtoCrudService } from 'Frontend/generated/endpoints';
...
function VersionView() {
  ...
  const [user, setUser] = useState<VersionDto>({} as VersionDto);
  const [selectedUserItems, setSelectedUserItems] = useState<VersionDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(VersionDtoModel, {
    onSubmit: async (userE) => {
      await VersionDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Version')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: VersionDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function DeleteRander({ item }: { item: VersionDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          VersionDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={VersionDtoCrudService}
              model={VersionDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'refDoctype', 'docname', 'data', 'idx']}
