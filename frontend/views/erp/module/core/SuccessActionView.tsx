...
import SuccessActionDto from 'Frontend/generated/com/itbd/application/db/dto/SuccessActionDto';
import SuccessActionDtoModel from 'Frontend/generated/com/itbd/application/db/dto/SuccessActionDtoModel';
...
import { DocFieldDtoCrudService, SuccessActionDtoCrudService } from 'Frontend/generated/endpoints';
...
function SuccessActionView() {
  ...
  const [user, setUser] = useState<SuccessActionDto>({} as SuccessActionDto);
  const [selectedUserItems, setSelectedUserItems] = useState<SuccessActionDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(SuccessActionDtoModel, {
    onSubmit: async (userE) => {
      await SuccessActionDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Success Action')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: SuccessActionDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/success-action/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: SuccessActionDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          SuccessActionDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={SuccessActionDtoCrudService}
              model={SuccessActionDtoModel}
              ref={autoGridRef}
...
export default SuccessActionView;
