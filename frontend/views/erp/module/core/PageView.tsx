...
import PageDto from 'Frontend/generated/com/itbd/application/db/dto/website/PageDto';
import PageDtoModel from 'Frontend/generated/com/itbd/application/db/dto/website/PageDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, PageDtoCrudService } from 'Frontend/generated/endpoints';
...
function PageView() {
  ...
  const [user, setUser] = useState<PageDto>({} as PageDto);
  const [selectedUserItems, setSelectedUserItems] = useState<PageDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(PageDtoModel, {
    onSubmit: async (userE) => {
      await PageDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Page')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: PageDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: PageDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          PageDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={PageDtoCrudService}
              model={PageDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'pageName', 'title', 'icon', 'module', 'idx']}
