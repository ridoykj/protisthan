...
import ScheduledJobTypeDto from 'Frontend/generated/com/itbd/application/db/dto/ScheduledJobTypeDto';
import ScheduledJobTypeDtoModel from 'Frontend/generated/com/itbd/application/db/dto/ScheduledJobTypeDtoModel';
...
import {
  ...
  ScheduledJobTypeDtoCrudService,
...
function ScheduledJobTypeView() {
  ...
  const [user, setUser] = useState<ScheduledJobTypeDto>({} as ScheduledJobTypeDto);
  const [selectedUserItems, setSelectedUserItems] = useState<ScheduledJobTypeDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ScheduledJobTypeDtoModel, {
    onSubmit: async (userE) => {
      await ScheduledJobTypeDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Scheduled Job Type')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as ScheduledJobTypeDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: ScheduledJobTypeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/scheduled-job-type/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: ScheduledJobTypeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          ScheduledJobTypeDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={ScheduledJobTypeDtoCrudService}
              model={ScheduledJobTypeDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'method', 'frequency', 'cronFormat', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/scheduled-job-type/_`);
        }}
      >
...
export default ScheduledJobTypeView;
