...
import DocShareDto from 'Frontend/generated/com/itbd/application/db/dto/doctypes/DocShareDto';
import DocShareDtoModel from 'Frontend/generated/com/itbd/application/db/dto/doctypes/DocShareDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, DocShareDtoCrudService } from 'Frontend/generated/endpoints';
...
function DocShareView() {
  ...
  const [user, setUser] = useState<DocShareDto>({} as DocShareDto);
  const [selectedUserItems, setSelectedUserItems] = useState<DocShareDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(DocShareDtoModel, {
    onSubmit: async (userE) => {
      await DocShareDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'DocShare')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: DocShareDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: DocShareDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          DocShareDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={DocShareDtoCrudService}
              model={DocShareDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'user', 'shareDoctype', 'shareName', 'idx']}
