...
import ReportDto from 'Frontend/generated/com/itbd/application/db/dto/reports/ReportDto';
import ReportDtoModel from 'Frontend/generated/com/itbd/application/db/dto/reports/ReportDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, ReportDtoCrudService } from 'Frontend/generated/endpoints';
...
function ReportView() {
  ...
  const [user, setUser] = useState<ReportDto>({} as ReportDto);
  const [selectedUserItems, setSelectedUserItems] = useState<ReportDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ReportDtoModel, {
    onSubmit: async (userE) => {
      await ReportDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Report')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: ReportDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function DeleteRander({ item }: { item: ReportDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          ReportDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={ReportDtoCrudService}
              model={ReportDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'reportName', 'refDoctype', 'isStandard', 'idx']}
