...
import DocTypeStateDto from 'Frontend/generated/com/itbd/application/db/dto/doctypes/DocTypeStateDto';
import DocTypeStateDtoModel from 'Frontend/generated/com/itbd/application/db/dto/doctypes/DocTypeStateDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, DocTypeStateDtoCrudService } from 'Frontend/generated/endpoints';
...
function DocTypeStateView() {
  ...
  const [user, setUser] = useState<DocTypeStateDto>({} as DocTypeStateDto);
  const [selectedUserItems, setSelectedUserItems] = useState<DocTypeStateDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(DocTypeStateDtoModel, {
    onSubmit: async (userE) => {
      await DocTypeStateDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'DocType State')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: DocTypeStateDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function DeleteRander({ item }: { item: DocTypeStateDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          DocTypeStateDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={DocTypeStateDtoCrudService}
              model={DocTypeStateDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'title', 'color', 'custom', 'idx']}
...
export default DocTypeStateView;
