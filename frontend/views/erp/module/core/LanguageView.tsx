...
import LanguageDto from 'Frontend/generated/com/itbd/application/db/dto/LanguageDto';
import LanguageDtoModel from 'Frontend/generated/com/itbd/application/db/dto/LanguageDtoModel';
...
import { DocFieldDtoCrudService, LanguageDtoCrudService } from 'Frontend/generated/endpoints';
...
function LanguageView() {
  ...
  const [user, setUser] = useState<LanguageDto>({} as LanguageDto);
  const [selectedUserItems, setSelectedUserItems] = useState<LanguageDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(LanguageDtoModel, {
    onSubmit: async (userE) => {
      await LanguageDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Language')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: LanguageDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function DeleteRander({ item }: { item: LanguageDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          LanguageDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={LanguageDtoCrudService}
              model={LanguageDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'languageCode', 'languageName', 'flag', 'idx']}
