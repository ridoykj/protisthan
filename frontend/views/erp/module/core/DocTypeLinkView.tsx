...
import DocTypeLinkDto from 'Frontend/generated/com/itbd/application/db/dto/doctypes/DocTypeLinkDto';
import DocTypeLinkDtoModel from 'Frontend/generated/com/itbd/application/db/dto/doctypes/DocTypeLinkDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, DocTypeLinkDtoCrudService } from 'Frontend/generated/endpoints';
...
function DocTypeLinkView() {
  ...
  const [user, setUser] = useState<DocTypeLinkDto>({} as DocTypeLinkDto);
  const [selectedUserItems, setSelectedUserItems] = useState<DocTypeLinkDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(DocTypeLinkDtoModel, {
    onSubmit: async (userE) => {
      await DocTypeLinkDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'DocType Link')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: DocTypeLinkDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function DeleteRander({ item }: { item: DocTypeLinkDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          DocTypeLinkDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={DocTypeLinkDtoCrudService}
              model={DocTypeLinkDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'linkDoctype', 'linkFieldname', 'group', 'custom', 'idx']}
...
export default DocTypeLinkView;
