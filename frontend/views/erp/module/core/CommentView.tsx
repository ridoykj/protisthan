...
import CommentDto from 'Frontend/generated/com/itbd/application/db/dto/cummunication/CommentDto';
import CommentDtoModel from 'Frontend/generated/com/itbd/application/db/dto/cummunication/CommentDtoModel';
...
import { CommentDtoCrudService, DocFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function CommentView() {
  ...
  const [user, setUser] = useState<CommentDto>({} as CommentDto);
  const [selectedUserItems, setSelectedUserItems] = useState<CommentDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(CommentDtoModel, {
    onSubmit: async (userE) => {
      await CommentDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Comment')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: CommentDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function DeleteRander({ item }: { item: CommentDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          CommentDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={CommentDtoCrudService}
              model={CommentDtoModel}
              ref={autoGridRef}
