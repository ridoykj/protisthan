...
import FileDto from 'Frontend/generated/com/itbd/application/db/dto/FileDto';
import FileDtoModel from 'Frontend/generated/com/itbd/application/db/dto/FileDtoModel';
...
import { DocFieldDtoCrudService, FileDtoCrudService } from 'Frontend/generated/endpoints';
...
function FileView() {
  ...
  const [user, setUser] = useState<FileDto>({} as FileDto);
  const [selectedUserItems, setSelectedUserItems] = useState<FileDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(FileDtoModel, {
    onSubmit: async (userE) => {
      await FileDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'File')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: FileDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function DeleteRander({ item }: { item: FileDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          FileDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              ...
              columnOptions={{
                ...
                isHomeFolder: {
                  header: 'Is Home Folder',
                  resizable: true,
                },
                isAttachmentsFolder: {
                  header: 'Is Attachments Folder',
                  resizable: true,
                },
                fileSize: {
                  header: 'File Size',
