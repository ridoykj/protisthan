...
import NavbarItemDto from 'Frontend/generated/com/itbd/application/db/dto/NavbarItemDto';
import NavbarItemDtoModel from 'Frontend/generated/com/itbd/application/db/dto/NavbarItemDtoModel';
...
import { DocFieldDtoCrudService, NavbarItemDtoCrudService } from 'Frontend/generated/endpoints';
...
function NavbarItemView() {
  ...
  const [user, setUser] = useState<NavbarItemDto>({} as NavbarItemDto);
  const [selectedUserItems, setSelectedUserItems] = useState<NavbarItemDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(NavbarItemDtoModel, {
    onSubmit: async (userE) => {
      await NavbarItemDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Navbar Item')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: NavbarItemDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function DeleteRander({ item }: { item: NavbarItemDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          NavbarItemDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={NavbarItemDtoCrudService}
              model={NavbarItemDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'itemLabel', 'itemType', 'route', 'hidden', 'idx']}
