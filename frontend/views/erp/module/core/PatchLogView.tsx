...
import PatchLogDto from 'Frontend/generated/com/itbd/application/db/dto/PatchLogDto';
import PatchLogDtoModel from 'Frontend/generated/com/itbd/application/db/dto/PatchLogDtoModel';
...
import { DocFieldDtoCrudService, PatchLogDtoCrudService } from 'Frontend/generated/endpoints';
...
function PatchLogView() {
  ...
  const [user, setUser] = useState<PatchLogDto>({} as PatchLogDto);
  const [selectedUserItems, setSelectedUserItems] = useState<PatchLogDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(PatchLogDtoModel, {
    onSubmit: async (userE) => {
      await PatchLogDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Patch Log')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: PatchLogDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function DeleteRander({ item }: { item: PatchLogDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          PatchLogDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={PatchLogDtoCrudService}
              model={PatchLogDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'skipped', 'idx']}
