...
import VariantFieldDto from 'Frontend/generated/com/itbd/application/db/dto/VariantFieldDto';
import VariantFieldDtoModel from 'Frontend/generated/com/itbd/application/db/dto/VariantFieldDtoModel';
...
import { DocFieldDtoCrudService, VariantFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function VariantFieldView() {
  ...
  const [user, setUser] = useState<VariantFieldDto>({} as VariantFieldDto);
  const [selectedUserItems, setSelectedUserItems] = useState<VariantFieldDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(VariantFieldDtoModel, {
    onSubmit: async (userE) => {
      await VariantFieldDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Variant Field')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: VariantFieldDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function DeleteRander({ item }: { item: VariantFieldDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          VariantFieldDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={VariantFieldDtoCrudService}
              model={VariantFieldDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'fieldName', 'idx']}
...
export default VariantFieldView;
