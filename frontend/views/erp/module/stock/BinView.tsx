...
import BinDto from 'Frontend/generated/com/itbd/application/db/dto/BinDto';
import BinDtoModel from 'Frontend/generated/com/itbd/application/db/dto/BinDtoModel';
...
import { BinDtoCrudService, DocFieldDtoCrudService } from 'Frontend/generated/endpoints';
...
function BinView() {
  ...
  const [user, setUser] = useState<BinDto>({} as BinDto);
  const [selectedUserItems, setSelectedUserItems] = useState<BinDto[]>([]);
  ...
  const {
    ...
    onSubmit: async (userE) => {
      await BinDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Bin')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: BinDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: BinDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          BinDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
