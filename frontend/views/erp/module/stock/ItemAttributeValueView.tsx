...
import ItemAttributeValueDto from 'Frontend/generated/com/itbd/application/db/dto/items/ItemAttributeValueDto';
import ItemAttributeValueDtoModel from 'Frontend/generated/com/itbd/application/db/dto/items/ItemAttributeValueDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, ItemAttributeValueDtoCrudService } from 'Frontend/generated/endpoints';
...
function ItemAttributeValueView() {
  ...
  const [user, setUser] = useState<ItemAttributeValueDto>({} as ItemAttributeValueDto);
  const [selectedUserItems, setSelectedUserItems] = useState<ItemAttributeValueDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ItemAttributeValueDtoModel, {
    onSubmit: async (userE) => {
      await ItemAttributeValueDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Item Attribute Value')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    ...
    },
    {
      ...
      onClick: () => {
        clear();
        setUser({} as ItemAttributeValueDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: ItemAttributeValueDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/item-attribute-value/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function DeleteRander({ item }: { item: ItemAttributeValueDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          ItemAttributeValueDtoCrudService.delete(name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={ItemAttributeValueDtoCrudService}
              model={ItemAttributeValueDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'attributeValue', 'abbr', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/item-attribute-value/_`);
        }}
      >
...
export default ItemAttributeValueView;
