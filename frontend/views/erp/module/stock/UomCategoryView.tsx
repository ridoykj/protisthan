...
import UomCategoryDto from 'Frontend/generated/com/itbd/application/db/dto/uoms/UomCategoryDto';
import UomCategoryDtoModel from 'Frontend/generated/com/itbd/application/db/dto/uoms/UomCategoryDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, UomCategoryDtoCrudService } from 'Frontend/generated/endpoints';
...
function UomCategoryView() {
  ...
  const [user, setUser] = useState<UomCategoryDto>({} as UomCategoryDto);
  const [selectedUserItems, setSelectedUserItems] = useState<UomCategoryDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(UomCategoryDtoModel, {
    onSubmit: async (userE) => {
      await UomCategoryDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'UOM Category')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: UomCategoryDto }) {
    const { name } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: UomCategoryDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          UomCategoryDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={UomCategoryDtoCrudService}
              model={UomCategoryDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'categoryName', 'idx']}
...
export default UomCategoryView;
