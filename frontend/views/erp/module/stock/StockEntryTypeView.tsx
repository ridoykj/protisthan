...
import StockEntryTypeDto from 'Frontend/generated/com/itbd/application/db/dto/stocks/StockEntryTypeDto';
import StockEntryTypeDtoModel from 'Frontend/generated/com/itbd/application/db/dto/stocks/StockEntryTypeDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, StockEntryTypeDtoCrudService } from 'Frontend/generated/endpoints';
...
function StockEntryTypeView() {
  ...
  const [user, setUser] = useState<StockEntryTypeDto>({} as StockEntryTypeDto);
  const [selectedUserItems, setSelectedUserItems] = useState<StockEntryTypeDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(StockEntryTypeDtoModel, {
    onSubmit: async (userE) => {
      await StockEntryTypeDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Stock Entry Type')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: StockEntryTypeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/stock-entry-type/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: StockEntryTypeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          StockEntryTypeDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={StockEntryTypeDtoCrudService}
              model={StockEntryTypeDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'purpose', 'idx']}
...
export default StockEntryTypeView;
