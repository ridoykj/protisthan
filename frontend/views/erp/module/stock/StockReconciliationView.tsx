...
import StockReconciliationDto from 'Frontend/generated/com/itbd/application/db/dto/stocks/StockReconciliationDto';
import StockReconciliationDtoModel from 'Frontend/generated/com/itbd/application/db/dto/stocks/StockReconciliationDtoModel';
...
import {
  ...
  StockReconciliationDtoCrudService,
...
function StockReconciliationView() {
  ...
  const [user, setUser] = useState<StockReconciliationDto>({} as StockReconciliationDto);
  const [selectedUserItems, setSelectedUserItems] = useState<StockReconciliationDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(StockReconciliationDtoModel, {
    onSubmit: async (userE) => {
      await StockReconciliationDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(
      pagination,
      filterGenerator('and', 'parent', 'Stock Reconciliation')
    ).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  const speedDial = [
    {
      ...
      onClick: () => {
        clear();
        setUser({} as StockReconciliationDto);
        setSelectedUserItems([]);
        setIsOpen(true);
      },
    },
  ...
  function ChildRedirect({ item }: { item: StockReconciliationDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/stock-reconciliation/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: StockReconciliationDto }) {
    return (
      <button
        type="button"
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={StockReconciliationDtoCrudService}
              model={StockReconciliationDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'differenceAmount', 'costCenter', 'idx']}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/stock-reconciliation/_`);
        }}
      >
        <FormLayout responsiveSteps={responsiveSteps} className="w-fit h-fit p-2">
          ...
          />
          <TextField label="Company" required {...{ colspan: 1 }} {...field(model.company)} />
          <TextField label="Purpose" required {...{ colspan: 1 }} {...field(model.purpose)} />
          <TextField
            label="Posting Date"
            required
            {...{ colspan: 1 }}
            {...field(model.postingDate)}
          />
        </FormLayout>
      </DialogFromRC>
    </>
