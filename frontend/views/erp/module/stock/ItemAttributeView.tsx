...
import ItemAttributeDto from 'Frontend/generated/com/itbd/application/db/dto/items/ItemAttributeDto';
import ItemAttributeDtoModel from 'Frontend/generated/com/itbd/application/db/dto/items/ItemAttributeDtoModel';
import Filter from 'Frontend/generated/dev/hilla/crud/filter/Filter';
import Matcher from 'Frontend/generated/dev/hilla/crud/filter/PropertyStringFilter/Matcher';
import Pageable from 'Frontend/generated/dev/hilla/mappedtypes/Pageable';
import { DocFieldDtoCrudService, ItemAttributeDtoCrudService } from 'Frontend/generated/endpoints';
...
function ItemAttributeView() {
  ...
  const [user, setUser] = useState<ItemAttributeDto>({} as ItemAttributeDto);
  const [selectedUserItems, setSelectedUserItems] = useState<ItemAttributeDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(ItemAttributeDtoModel, {
    onSubmit: async (userE) => {
      await ItemAttributeDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'Item Attribute')).then((result) => {
      setUiField(result);
    });
  }, []);
  ...
  function ChildRedirect({ item }: { item: ItemAttributeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        onClick={(e) => {
          setUser(item);
          read(item);
          navigate(`/m/item-attribute/${name}`);
        }}
      >
        {name}
      </button>
  ...
  function deleteRander({ item }: { item: ItemAttributeDto }) {
    const { name } = item;
    return (
      <button
        type="button"
        ...
        title="Delete"
        onClick={(e) => {
          ItemAttributeDtoCrudService.delete(item.name).then((result) => {
            setGridRefresh(!gridRefresh);
          });
        }}
      >
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={ItemAttributeDtoCrudService}
              model={ItemAttributeDtoModel}
              ref={autoGridRef}
              className="h-full w-full overflow-auto bg-white/40"
              visibleColumns={['name', 'attributeName', 'idx']}
...
export default ItemAttributeView;
