...
import WarehouseDto from 'Frontend/generated/com/itbd/application/db/dto/warehouse/WarehouseDto';
import WarehouseDtoModel from 'Frontend/generated/com/itbd/application/db/dto/warehouse/WarehouseDtoModel';
...
function WarehouseView() {
  ...
  const [user, setUser] = useState<WarehouseDto>({} as WarehouseDto);
  const [selectedUserItems, setSelectedUserItems] = useState<WarehouseDto[]>([]);
  ...
  const {
    ...
    validate,
    addValidator,
  } = useForm(WarehouseDtoModel, {
    onSubmit: async (userE) => {
      await WarehouseDtoCrudService.save(userE)
        .then((result) => {
          clear();
  ...
  useEffect(() => {
    DocFieldDtoCrudService.list(pagination, filterGenerator('and', 'parent', 'warehouse')).then(
      (result) => {
        setUiField(result);
      }
    );
  ...
  function ChildRedirect({ item }: { item: WarehouseDto }) {
    const { name, warehouseName } = item;
    return (
      <button
        type="button"
  ...
  function deleteRander({ item }: { item: WarehouseDto }) {
    return (
      <button
        type="button"
      ...
      <>
        <div className="flex flex-col h-full items-stretch ">
          ...
          <div className="h-full mx-2 mb-2 bg-white p-3 rounded-xl border">
            <AutoGrid
              service={WarehouseDtoCrudService}
              model={WarehouseDtoModel}
              ref={autoGridRef}
    ...
    <>
      ...
      <DialogFromRC
        ...
        onNavigate={() => {
          setIsOpen(false);
          clear();
          navigate(`/m/warehouse/_`);
        }}
      >
        <FormLayout responsiveSteps={responsiveSteps} className="w-fit h-fit p-2">
          <TextField label="Warehouse Name" {...{ colspan: 1 }} {...field(model.warehouseName)} />
          <TextField label="Company" {...{ colspan: 1 }} {...field(model.company)} />
        </FormLayout>
      </DialogFromRC>
    </>
  );
