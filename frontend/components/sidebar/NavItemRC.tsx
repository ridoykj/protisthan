...
const navLinkClasses = ({ isActive }: any) => {
    return `${isActive ? 'bg-indigo-100 text-indigo-700 rounded-md ' : ''}${navCss}`;
};
...
function DropdownButton({ name, icon, isDropdownOpen, toggleDropdown }: { name: string, icon: any, isDropdownOpen: boolean, toggleDropdown: () => void }) {
    return (
        <button type="button" className={`${navCss} w-full flex justify-between items-center h-10 pr-6 ${isDropdownOpen ? 'shadow-[0_5px_5px_#dfe7ff]' : ''}`} onClick={toggleDropdown}>
